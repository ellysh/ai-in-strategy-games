...
Если поменяются условия игры, то изменится и её решение. Допустим, Работодатель и Работник заключают контракт, имеющий юридическую силу. Тогда Работодатель сможет потребовать от Работника компенсации за плохую работу через суд. В этом случае выигрыши меняются. В исходе узла E Работник получит не 3, а меньше: это зависит от суммы штрафа. Тогда ему станет невыгодно отвечать плохой работой на высокую зарплату.
...
Игра продолжается до тех пор, пока на столе не накопится 100 монет. Последний ход делает Боб в узле X. Он выбирает: или взять 100 монет, или пропустить ход. Если Боб пропускает ход, игра заканчивается в узле Z. Тогда он и Алиса ничего не получают. Если Боб берёт монеты, игра заканчивается в узле Y. Оба игрока знают, что ход Боба в узле X будет последним.
...
Перейдём к предпоследнему ходу Алисы. Она решает: брать ли ей 90 монет или пропустить ход. Алиса знает, что на последнем ходу в узле X Боб возьмёт монеты. Поэтому она должна сделать это раньше и взять 90 монет на предпоследнем ходу.
...
Продолжим это рассуждение назад до первого хода. Тогда окажется, что Алиса должна взять 10 монет в начальном узле A. В противном случае Боб возьмёт 20 монет уже на втором ходу в узле C.
...
Если участники могут договориться, решение игры изменится. В этом случае игра становится кооперативной. Оба участника должны ждать, пока сумма на столе не станет максимальной. Тогда второй игрок забирает монеты на последнем ходу в узле X. После этого он делит выигрыш с первым игроком пополам.
...
Наилучший выбор для Чака — тоже дать показания. Следовательно, решением "дилеммы заключённого" будет концевой узел G с выигрышами участников -2 и -2.
...
У Чака может быть одно из двух убеждений относительно стратегии Мэллори:
...
Парадокс объясняют так: равновесие Нэша в "дилемме заключённого" не является оптимумом по Парето. [**Оптимум по Парето**](https://ru.wikipedia.org/wiki/Эффективность_по_Парето) — это состояние системы, при котором суммарное благосостояние её участников достигает максимума. Любое изменение этого состояния ухудшает благосостояние хотя бы одного участника.
...
Обозначим буквой p вероятность того, что атакующий выберет чистую стратегию "удар вправо". Тогда он выберет стратегию "удар влево" с вероятностью `(1-p)`.

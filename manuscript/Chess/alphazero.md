...
### 3.7.2 Поиск по дереву методом Монте-Карло
...
#### 3.7.2.3 Преимущества и недостатки MCTS
...
### 3.7.3 Свёрточная нейронная сеть
...
#### 3.7.3.2 Применение функции активации
...
4. Выбор признаков. Функция активации позволяет модели концентрироваться на наиболее важных и отличительных признаках во входных данных. Она выборочно увеличивает или уменьшает определённые элементы карты признаков.
...
Выполним операцию подвыборки по среднему значению для карты признаков из нашего примера. На первом шаге окно подвыборки перекроет следующую его часть:
{width: "40%"}
![](images/Chess/pooling-source-image-5x5-highlight-first.png)
...
I> [**Темп обучения**](https://en.wikipedia.org/wiki/Learning_rate) (learning rate) — это настраиваемый параметр алгоритма обучения. Он определяет размер шага на каждой итерации при движении к минимуму функции потерь. **Функция потерь** (loss function) определяет стоимость неправильного принятия решений на основе наблюдаемых данных.
...
### 3.7.4 Нейронная сеть AlphaZero
...
1. **Вероятность каждого хода** (policy или move probability), возможного в заданной позиции. Этот вывод совмещает в себе генератор ходов и функцию оценки традиционного шахматного движка. Вероятность хода является по сути его оценкой: чем она выше, тем лучше ход.
...
#### 3.7.4.3 Архитектура сети

AlphaZero и AlphaGo Zero используют одну и ту же архитектуру нейронной сети. Команда DeepMind подробно описала её в статье, посвящённой AlphaGo Zero. Статья называется "Освоение го без человеческих знаний" (["Mastering Go without Human Knowledge"](https://discovery.ucl.ac.uk/10045895/1/agz_unformatted_nature.pdf)).
...
### 3.7.5 Результаты AlphaZero

Команда DeepMind предложила принципиально новый подход к разработке шахматных движков. Традиционные движки полностью полагаются на экспертные знания профессиональных шахматистов. Эти знания хранятся в дебютных книгах, таблицах эндшпилей, эвристиках алгоритма поиска и функции оценки. Именно к ним обращается движок, когда выбирает лучший ход.

AlphaZero обучается игре в шахматы самостоятельно. Модель команды DeepMind не заимствует никаких шахматных знаний у игроков. Система сама собирает и осваивает шаблоны позиций фигур. Они хранятся в параметрах нейронной сети. Полагаясь на эти шаблоны, AlphaZero выбирает лучшие ходы.

AlphaZero отличается от своих предшественников не только способом получения шахматных знаний. Модель работает с деревом игры принципиально иначе. Основанные на алгоритме минимакс движки тратят большую часть времени на перебор возможных ходов. Их функция оценки работает чрезвычайно быстро. Благодаря этому, они просчитывают дерево игры на глубину, недоступную профессиональным игрокам. Так движки находят сильные тактические комбинации, которые дают преимущество и приводят к победе.

AlphaZero большую часть времени тратит не на перебор позиций, а на их оценку. Дело в том, что каждое обращение к нейронной сети занимает время и требует значительных вычислительных ресурсов. Поэтому AlphaZero на каждом ходу анализирует лишь несколько перспективных вариантов и позиций. Система делает ставку на качество анализа, а не на количественные показатели вроде числа просмотренных ходов.

AlphaZero продемонстрировала новый уровень игры в партиях против людей и других движков. Система исполнила несколько новых дебютов. Некоторые из них были малопопулярны и считались бесперспективными. Другие были совершенно новыми и ещё не исследованными теоретиками шахматной игры. Игры AlphaZero показали потенциал определённых позиций и начал. Это расширило существующие границы и понимание дебютной теории.

AlphaZero также показала новый стиль игры. В нём важна мобильность фигур и их координация. Этот стиль дал новые идеи для шахматной стратегии и тактики. Он позволил по-новому взглянуть на динамику игры.

Наследником идей AlphaZero считается шахматный движок Leela Chess Zero. Его нейронная сеть оптимизирована только для игры в шахматы. Она обучается алгоритмом с подкреплением самостоятельно, так же как и сеть AlphaZero. Но в отличие от команды DeepMind разработчики добавили в Leela Chess Zero дебютную книгу и таблицы эндшпилей. Благодаря этому, система стала достойным конкурентом для традиционных шахматных движков наподобие Stockfish. Её успехи заставили разработчиков всех шахматных программ обратить внимание на методы машинного обучения.

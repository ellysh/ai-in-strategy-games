...
Перед тем как приступить к деталям, подумаем над следующим вопросом. Как сеть, оптимизированная для распознавания изображений, может оценивать шахматные позиции?
...
В разделе 3.3.3 мы познакомились с моделью шаблонов Герберта Саймона. Согласно ей, профессиональные шахматисты иерархически упаковывают информацию о позиции. Так они анализируют не отдельные фигуры, а их сочетания.
...
На какие именно признаки ориентируется обученная модель AlphaZero, сказать невозможно. Они обобщаются в высокоуровневые шаблоны. Похожие шаблоны распознают профессиональные игроки. Это сходство приближает AlphaZero к действиям профессиональных шахматистов. Поведение системы намного ближе к человеческому, чем у традиционных движков наподобие Stockfish.
...
1. **Вероятность каждого ход** (move probability), возможного в заданной позиции. Этот вывод совмещает в себе генератор ходов и функцию оценки традиционного шахматного движка. Вероятность хода является по-сути его оценкой: чем она выше, тем лучше ход.

2. **Результат партии** (value) указывает вероятность победы белых, чёрных или ничьи в заданной позиции.

Теперь поговорим об алгоритме поиска. Для выбора хода в текущей позиции AlphaZero использует поиск методом Монте-Карло. При этом нейронная сеть определяет направление, в котором развёртываются узлов дерева игры. Она применяется на двух шагах алгоритма MCTS:

1. На шаге selection нейронная сеть определяет, какой узел дерева выбрать для исследования. Он соответствует позиции после хода, который имеет наибольшую вероятность. Это первый вывод нейронной сети.

2. На шаге simulation алгоритм MCTS не проигрывает партию, выбирая случайные ходы. Вместо этого AlphaZero узнаёт наиболее вероятный результат партии из второго вывода нейронной сети.
...
2. Результат партии в каждой проверенной позиции, который предсказала нейронная сеть.

3. Вероятности ходов в каждой проверенной позиции, которые вернула сеть.

4. Реальный результат партии.

Алгоритм обучения использует эти данные, чтобы скорректировать параметры нейронной сети AlphaZero. Эта корректировка происходит после каждой партии, которую модель сыграла сама с собой.

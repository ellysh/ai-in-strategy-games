...
Команда DeepMind опубликовала статью ["Mastering Chess and Shogi by Self-Play with a General Reinforcement Learning Algorithm"](https://arxiv.org/pdf/1712.01815.pdf) с описанием своей системы. В ней сказано, что нейронная сеть имеет 119 входов. Все эти входы мы рассмотрели.
...
Архитектуры нейронных сетей AlphaZero и AlphaGo Zero очень похожи. Устройство AlphaGo Zero подробно описывает статья команды DeepMind под названием ["Mastering Go without Human Knowledge"](https://discovery.ucl.ac.uk/10045895/1/agz_unformatted_nature.pdf).
...
Входные данные сети (Input) представляют собой стек из 119 матриц размером 8x8. Их последовательно обрабатывают несколько блоков нейронной сети. Познакомимся с ними в порядке следования.
...
Первый блок свёртки (Convolutional block) вызывает несколько вопросов. Во-первых, если на входе стек из 119 матриц, то как их обрабатывают 256 фильтров свёрточного слоя (Convolution A)? В этом случае нельзя применить свёртку по объёму. При такой операции число **плоскостей** (plane) исходного изображения должно быть равно числу ядер. В результате все плоскости исходного изображения сводятся в одну плоскость карты признаков. В случае AlphaZero это решение неприемлемо. Каждая из 119 матриц несёт важную информацию и их данные не должны смешиваться между собой.

Вместо свёртки по объему происходит обычная свёртка. При этом каждое из 256 ядер применяется к каждой из 119 входных матриц. В результате получается 119 наборов по 256 карт признаков в каждом. Затем каждый набор из 256 карт признаков сворачивается в одну карту признаков. Для этого можно, например, просто просуммировать их элементы. То есть элемент (1,1) результирующей карты признаков равен сумме всех элементов (1,1) из набора 256 карт признаков. Так на выходе из свёрточного слоя получается стек из 119 карт признаков.

Второй вопрос — какой размер имеет каждая из 119 карт признаков после операции свёртки? Если применить фильтры без выравнивания, это приведёт к потере данных. Информация об элементах на границах каждой входной матрицы потеряется. Это допустимо при обработке изображений, но совершенно неприемлемо для матриц с расположением фигур на доске. Поэтому перед обработкой каждой входной матрицы выполняется выравнивание. В результате все 119 выходных карт признаков имеют тот же размер 8x8.
...
2. **Второй поток данных**: остаточное соединение передаёт входные данные блока на его выход, минуя все слои.
...
При обучении такой глубокой нейронной сети как у AlphaZero неизбежна проблема исчезающего градиента. Остаточные блоки решают эту проблему.

Всего в сети 19 остаточных блоков. Они следуют друг за другом. Последний из них выдаёт стек из 119 карт признаков размером 8x8. Они отличаются от выходных карт признаков блока свёртки (Convolutional block) тем, что содержат шаблоны самого высокого уровня абстракции. Как и в случае CNN сетей для распознавания изображений, выяснить суть этих шаблонов крайне сложно.
...
4. **Операция сглаживания** (Flatten). После свёртки типа B (Convolution B) получается стек из 119 карт признаков размером 8x8. Операция сглаживания приводит их в формат одномерного массива, который нужен следующему далее линейному слою.
...
Вторая голова вычисляет ожидаемый результат партии (Value head). Она последовательно выполняет следующие операции:
...
4. **Операция сглаживания** (Flatten). После свёртки типа C (Convolution C) получается стек из 119 карт признаков размером 8x8. Операция сглаживания приводит их в формат одномерного массива, который нужен следующему далее линейному слою.

5. **Линейный полносвязный слой типа B** (Dense B) сводит входные данные в одномерный массив из 256 элементов. Это входной формат следующего далее линейного слоя.

6. **Функция активации ReLU** (Activation function ReLU).

7. **Линейный полносвязный слой типа C** (Dense C) сводит входной массив из 256 элементов к одному вещественному числу.
...
Мы рассмотрели схему нейронной сети AlphaZero и все операции над входными данными, которые она выполняет.

...
На практике чаще всего применяется третья реализация алгоритма градиентного спуска. Она представляет собой компромиссное решение с хорошими характеристиками. Из-за популярности этого подхода мини-пакеты для простоты называют просто пакетами.
...
Пакетная нормализация корректирует входные данные каждого слоя сети. Это поддерживает в них стабильное среднее значение и дисперсию. Таким образом пакетная нормализация гарантирует, что входные данные слоёв будут согласованы на протяжении всего обучения. 
...
5. Повышает скорость обучения, поскольку позволяет использовать более высокий темп обучения. Часто увеличение темпа обучения приводит к взрыву или исчезновению градиента, но пакетная нормализация предотвращает эти проблемы.

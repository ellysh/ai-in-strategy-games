...
Чтобы выбрать потенциальные ответы оппонента на шаге expansion, программе снова нужна некоторая стратегия. Она может быть той же самой, которая использовалась на первом шаге selection.
...
2. Для узла B программа должна выбрать, какой из возможных следующих ходов анализировать. Для этого она снова применяет стратегию выбора перспективного хода. Стратегия указывает на ход, ведущий к позиции в узле F. Этот ход ещё не рассматривался, поэтому программа выполняет шаг expansion и добавляет в дерево новый узел F.
...
Формула UCT предлагает альтернативу для стратегии финального выбора хода. Ранее мы рассматривали шаги поиска методом Монте-Карло. После выполнения backpropagation мы получили состояние дерева как на иллюстрации 3-38. Из этого состояния мы сделали вывод, что наилучшим ходом будет B. В этом узле у программы самый высокий коэффициент побед.

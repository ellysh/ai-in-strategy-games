...
С точки зрения интеллектуального агента игра в шахматы — это типичная проблемная среда. Она однозначно определяет типы агентов, которые способны в ней работать. Поэтому нам нужно выяснить все свойства проблемной среды и установить её тип. Для этого обратимся к таблице 2-4 с признаками классификации сред.

Агент знает расположение всех фигур на доске в любой момент времени. Кроме этого он имеет полную информацию обо всех произошедших игровых событиях. Это происходит потому, что в шахматах нет скрытых состояний. Следовательно, шахматы — это **полностью наблюдаемая** среда.
...
В шахматах каждое действие агента влияет на будущие состояния среды. Он не может принимать решения только исходя из текущей позиции. Вместо этого агент должен планировать события наперёд. Поэтому шахматы являются **последовательной** средой.

В шахматах игроки делают ходы друг за другом. Позиция на доске меняется только после очередного хода. Это означает, что пока агент принимает решения, среда остаётся неизменной. Такое поведение характерно для  статических сред. С другой стороны, на турнирных партиях каждому участнику отводится определённое время на все ходы. Когда оно заканчивается, игроку засчитывается поражение. Поэтому показатели производительности агента меняются с течением времени. Это означает, что шахматы являются **полудинамической** средой.

В шахматах фигуры могут находиться только на определённых полях доски. Поэтому агент способен однозначно различать все возможные состояния игры. Количество этих состояний огромно, но конечно. Точно так же различимы и конечны игровое время и ходы участников. Следовательно, шахматы — это **сложная дискретная** среда.

В каждой шахматной партии только два участника. Победа одного из них означает поражение другого. Таким образом показатели производительности игроков обратно пропорциональны. Из этого следует, что шахматы — это **мультиагентная конкурентная** среда.
...
Согласно расчётам Клода Шеннона, существует около 10^120^ возможных позиций на шахматной доске. Это число несоизмеримо больше 5478 — числа возможных состояний поля в крестиках ноликах. Соотношение чисел 10^120^ и 5478 показывает насколько больше таблица правил для шахмат, чем для крестиков-ноликов. Современные компьютерные технологии просто не позволяют сохранить такой объем информации.

...
Может ли играть в шахматы какой-то из рефлексных агентов? Начнём с **простого рефлексного агента**. Он полностью игнорирует последовательность актов восприятия. При выборе действия агент учитывает только текущее состояние среды. Он не рассматривает последствия своих действий и будущие возможные состояния среды.

Теоретически, простой рефлексный агент может работать в среде, похожей на шахматы. Пример — крестики-нолики с классическим полем 3 на 3 клетки. Крестики-нолики считаются [решённой игрой](https://en.wikipedia.org/wiki/Solved_game). Для каждого игрока найдена оптимальная стратегия, которая приводит к наилучшему результату. Если оба игрока придерживаются своей оптимальной стратегии, партия всегда заканчивается вничью.

Простой рефлексный агент может играть в крестики нолики, если у него есть таблица правил со следующей информацией:
...
Теперь перейдём от теории к практики. Выясним размер таблицы правил с реализацией оптимальной стратегии. Всего поле игры имеет 9 клеток. Каждая клетка может иметь одно из трёх состояний: пустая, нолик, крестик. Поэтому есть 3^9^ = 19683 состояний игрового поля. Из них только 5478 состояний удовлетворяют правилам игры. Таким образом, таблица правил простого рефлексного агента должна иметь 5478 строк.

Каждая строка таблицы правил — это состояние игрового поля из 9 клеток плюс соответствующее ему действие агента. Действие агента можно закодировать в 9 битах. Это ход в одну из девяти клеток. Состояние поля можно сохранить в `9 * 3 = 27` битах. Это три состояния для каждой клетки: пустая, крестик или нолик. Итого мы получаем размер таблицы `5478 * (27 + 9) = 197208` бит или 24651 байт.

Мы выяснили, что для игры в крестики-нолики простому рефлексному агенту нужна таблица правил размером около 24 Кбайт. Теперь оценим размер такой таблицы для шахмат. Согласно расчётам Клода Шеннона, существует около 10^120^ возможных позиций на шахматной доске. Для удобства сравнения грубо округлим число возможных состояний в крестиках-ноликах до 10^4^. Тогда получается, что в шахматах состояний игрового поля на 116 порядков больше.

Для простоты мы не будем учитывать, что запись в таблице правил для шахмат занимает больше памяти, чем для крестиков-ноликов. Причина в том, что на шахматной доске больше полей, а у игроков больше вариантов действий. Даже несмотря на это, простому рефлексному агенту нужна таблица правил размером около 10^120^ байт, чтобы играть в шахматы. Работа с таким объёмом информации выглядит недостижимой для современных компьютерных технологий.

У простого рефлексного агента, играющего в шахматы, есть ещё одна проблема. Шахматы до сих пор не решены. Это означает, что оптимальные стратегии для белых и чёрных неизвестны. Поэтому если бы даже компьютерные технологии позволили работать с таблицей правил размером 10^120^ байт, её содержимое остаётся загадкой.
...
Внутреннее состояние решает проблему с запоминанием прошлых ходов. Но оно никак не поможет агенту в планировании действий наперёд. Поэтому рефлексный агент с внутренним состоянием по прежнему не способен играть в шахматы.

Модель помогает рефлексному агенту прогнозировать изменения среды. Но она работает только для простых проблемных сред. Шахматы — это сложная среда. При попытке применить модель к ней возникает комбинаторный взрыв. Количество состояний среды и переходов между ними оказывается слишком велико для просчёта с помощью модели. Поэтому такой рефлексный агент также не способен играть в шахматы.

Мы пришли к выводу, что все рефлексные агенты не подходят для игры в шахматы.

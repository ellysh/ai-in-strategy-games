...
Мы обсудили три стратегии поиска ошибок в шахматных программах. Возникает вопрос об их универсальности. Можно ли применить эти стратегии к поиску ошибок в интеллектуальном агенте любого типа и для любой игры?

Первая стратегия ручного несистематического поиска очень проста. Если интеллектуальный агент способен действовать в какой-то стратегической игре, человек может провести с ним партию. Во время партии могут проявиться слабые стороны агента. Они указывают на недочёты в его управляющей программе. Таким образом, стратегия ручного несистематического поиска работает для любого типа агентов и любых игр. Её могут применять как разработчики, так и игроки.

Вторая стратегия ручного систематического поиска ошибок сложнее. Чтобы её применить, нужна возможность следовать повторяемым путём для воспроизведения ошибок. Например, шахматные программы, которые только играют партию от начала и до конца, этого не позволяют. Поэтому для них нужны специальные инструменты тестирования. Такой инструмент задаёт произвольную позицию фигур и передаёт её на вход шахматной программе или её части (функции оценки). Это даёт возможность повторять одни и те же тесты. Тестами будут наборы заранее подготовленных позиций.

Ещё одна возможность следовать повторяемым путём для воспроизведения ошибки в компьютерных шахматах — расширить правила игры. Правило, разрешающее брать ход назад, позволяет создать нужную позицию фигур на доске.

Если интеллектуальный агент играет в достаточно простую игру, её правила могут позволять следовать повторяемым путём для воспроизведения ошибки. В этом случае специальных инструментов и расширения правил не нужно. Если игра сложная, то наличие таких инструментов обязательно. Без них любая попытка применить ручной систематический поиск закончится отыгрыванием полных партий против агента. Другими словами, мы возвращаемся к стратегии ручного несистематического поиска.

Мы пришли к выводу, что стратегия РСН для поиска ошибок в интеллектуальных агентах применима только в определённых условиях. При этом тип проверяемого интеллектуального агента не важен.

Третья стратегия автоматического систематического поиска самая сложная. Есть несколько условий для её применения. Первое условие: агент не должен существовать в единственном экземпляре. Только в этом случае можно проводить партии между его разными версиями. Для примера допустим, что агент — это единственный специализированный суперкомпьютер. В этом случае автоматический систематический поиск неприменим. Если же агент — это программа для обычного ПК, стратегия АСП может сработать.

Второе условие: нужна система автоматического тестирования. Базовая функция этой системы — проводить партии между разными версиями интеллектуального агента. Не все игры позволяют такую автоматизацию. Если игра сложная и одна партия занимает значительное время, система автоматического тестирования будет неэффективна. Она не даст практически никаких преимуществ перед ручным систематическим поиском ошибок.

Подведём итог. Применимость стратегий поиска ошибок в интеллектуальных агентах зависит от двух вещей. Во-первых, это тип игры. Правила сложных игр могут стать препятствиями для использования стратегий РСП и АСП. В случае РСП проблему можно решить с помощью специальных инструментов и расширения правил игры. Для АСП критическим условием становится время, за которое два агента могут сыграть одну партию. 

Во-вторых,важен способ реализации агента. Если агент существует в единственном экземпляре, стратегия АСП неприменима. При этом внутреннее устройство никак не ограничивает использование стратегий поиска ошибок.

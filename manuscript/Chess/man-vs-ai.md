...
Вот пример нескольких настольных игр: крестики-нолики, шахматы, го. Все они с точки зрения теории игр имеют одинаковые характеристики:
...
Несмотря на это, компьютерные программы для игры в них на уровне человека появились со значительным временным интервалом. Эти игры различаются размером дерева игры. Можно сказать, что у них разная **вычислительная сложность**.

Кроме вычислительной сложности, игра может иметь **сложные правила**. Для человека и для машины сложности создают разные правила. Например, игры со следующими характеристиками считаются трудными для машин:
...
В таких играх программе сложно учитывать все аспекты. Этих аспектов очень много. Поэтому сама программа должна быть достаточно сложной, чтобы охватить возможные состояния игры и действия участников.
...
Теперь применим автоматический систематический поиск ошибок. Например, это может быть какой-то эволюционный алгоритм. Программа будет играть сама с собой и исправлять ошибки. Но во время такого обучения конями ходить она не будет из-за принципиального ограничения. В результате программа научится очень хорошо обращаться с теми фигурами, которыми она играла. Но будет ли такая программа хорошо играть в шахматы? Скорее всего нет.
...
Рассмотрим, какие возможности есть у человека в игре с шахматным движком. Для определённости будем говорить только о Stockfish.

Рассуждения об ошибках в шахматных программах привели нас к трём критериям для их оценки:
...
Теперь перевернём ситуацию. Оценим по этим же самым критериям профессионального шахматиста:
...
Кто сегодня играет в шахматы сильнее: человек или машина? Ответ очевиден — машина. Давайте изменим наш вопрос. Есть ли хоть какая-то возможность у человека выиграть партию в шахматы против машины?

Дальнейшие рассуждения не имеют отношения к честной игре в шахматы. Мы рассмотрим все возможные действия, которые человек может в принципе предпринять.
...
2. Игры в ходе обучения с подкреплением системы Leela Chess Zero.
...
Второй сценарий: шахматист знает, что играет против программы. Во-первых, он фиксирует её версию и компьютер на которой она работает. Так он добивается повторяемой среды. Во-вторых, он предполагает, какие у программы в теории могут быть слабые стороны. Например, если это ранняя версия Stockfish, она может ошибаться в стратегии. В этом случае во время многократных игр человек целенаправленно проверяет программу в сложных позиция, требующих понимания шахматной стратегии. Рано или поздно он надеется подловить движок на ошибке.

Этот сценарий соответствует ручному систематическому поиску ошибок. В нём у шахматиста есть информация, в каком направлении надо искать слабость программы. К сожалению, если речь идёт о современных версиях Stockfish, такой подход вряд ли приведёт к победе. Причина в том, что стратегия поиска ошибок у разработчиков Stockfish сильнее. Они уже нашли и исправили все простые ошибки, которые можно было бы найти ручным поиском.

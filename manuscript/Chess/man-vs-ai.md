...
Теперь поговорим о противостоянии человека и машины в произвольной игре. Как и в случае компьютерных шахмат пусть против машины выступит не один игрок, а команда экспертов. Её состав может быть разным.

Шансы на победу команды экспертов зависят от ряда факторов. Основные из них следующие:

1. Правила и сложность игры.

2. Состав команды, которая играет против машины, и уровень её участников. Это может быть один профессиональный игрок или команда экспертов разного профиля.

3. Информация о машине, которая доступна команде экспертов. 

4. Ресурсы, которыми располагает команда экспертов. К ним относятся вычислительные мощности, время и специальные инструменты.

План экспертов на матч с машиной зависит от этих факторов. Они определят, насколько качественно специалисты выполнят каждый пункт плана. Информация является важнейшим аспектом. В общем случае чем больше эксперты знают или могут узнать о машине, тем эффективнее будет их подготовка к матчу.

План команды экспертов на подготовку и матч с машиной выглядит следующим образом:
...
**Первый шаг** — собрать информацию о машине. Успех зависит от доступности этой информации и от состава команды экспертов. Если в команду входят специалисты по разным компонентам машины, они смогут эффективнее собирать сведения и использовать их.
...
1. Какую стратегию поиска ошибок применяли разработчики?
...
Ответ на первый вопрос позволит оценить ресурсы, которые нужны команде экспертов. Чем сильнее стратегия поиска ошибок разработчиков машины, тем больше усилий и времени потребуется экспертам на подготовку. В некоторых случаях, у них может просто не оказаться достаточных ресурсов. Тогда лучше сразу отказаться от матча, потому что победа над машиной в таких условиях маловероятна.
...
Некоторые из этих пунктов могут остаться неизвестными. В худшем случае команде экспертов ни удастся выяснить ни один из них. Но даже тогда у экспертов есть шанс успешно подготовиться к матчу.
...
Перед тем как искать ошибки, их надо классифицировать по каким-то признакам. Классификация упорядочивает процесс поиска и позволяет эффективно разделить задачи между специалистами разного профиля, которые входят в команду экспертов.
...
В качестве отправной точки для классификации ошибок, можно рассмотреть типичные изъяны известных машины для настольных игр. Эти изъяны следующие:
...
2. **Машина не справляется с вычислительной сложностью игры**. Пример такой проблемы — эффект горизонта в ранних шахматных программах. Они просчитывают позиции до четырёх ходов вперёд. Это означает, что программы не видят планы и комбинации оппонента на пять и более ходов вперёд.

3. **Логические ошибки при выборе действия**. Рассмотрим пример. Пусть машина или её часть реализована как рефлексный агент. В этом случае типична ошибка интерпретации текущего состояния среды. Если два состояния похожи по некоторым признакам, машина может их путать. Тогда она будет выбирать неверное действие.

4. **Ошибки обучения модели**. Если машина представляет собой обучающегося агента, стоит проверить типичные ошибки алгоритма обучения: низкая производительность модели, переобучение, недообучение, плохая обобщающая способность и т.д.
...
Это не универсальная классификация. Но она предлагает типы ошибок, которые характерны для машин с разной реализацией. В некоторых случаях её может оказаться достаточно.

Мы уже рассмотрели сценарии игры команды экспертов против шахматной программы. Эти сценарии можно обобщить на любую игру. Кратко повторим здесь условия для выполнения каждого сценария.

Сценарий РНП со стороны экспертов выполняется в следующем случае:
...
Команда экспертов может применить сценарий РСП в следующем случае:

1. В команду экспертов входят игроки и специалисты по аналогичным машинам.
...
Сценарий АСП выполняется экспертами в следующем случае:

1. В команду экспертов входят игроки, специалисты по аналогичным машинам, тестированию и статистике.
...
Если эксперты выбирают один из трёх сценариев и все его условия выполняются,  у них есть хорошие шансы на победу в матче с машиной. Чем сильнее стратегия поиска ошибок команды экспертов, тем выше эти шансы.

**Третий шаг** плана — научиться воспроизводить найденные ошибки машины. Успех этого шага зависит от доступа к экземпляру машины, обмена информацией внутри команды экспертов, уровня входящих в неё игроков и ресурсов, которыми они располагают.

Очевидно, что если у команды экспертов нет доступа к экземпляру машины, они не смогут до матча проверить найденные изъяны в её работе. В этом случае третий шаг плана оказывается невыполним.
...
Уровень игроков команды экспертов важен. Чем он выше, тем разнообразнее ошибки машины, которые они могут воспроизводить и эксплуатировать. Игрок новичок способен понять и воспользоваться грубой ошибкой оппонента. Но только профессионал сможет заметить и извлечь выгоду из незначительного стратегического просчёта противника.

Команда экспертов располагает какими-то ресурсами. Их дефицит значительно осложняет воспроизведение ошибок машины. Рассмотрим пример. Допустим, что машина отыгрывает только полные партии от начала до конца. Игра достаточно сложная и одна партия занимает много времени. Предположим, что команда экспертов нашла десять потенциальных проблем в поведении машины. Чтобы воспроизвести и проверить их все, игрокам команды нужно значительное время. Если до начала матча этого времени нет, им придётся выбрать несколько самых перспективных ошибок и работать только с ними.
...
Во время матча также важно вмешательство разработчиков машины. Они могут вносить исправления между партиями. В этом случае у команды экспертов должен быть достаточный набор ошибок для эксплуатации. Если ошибок недостаточно, разработчики могут исправить их все. Тогда команда экспертов потеряет все преимущества, которые дала им подготовка к матчу.

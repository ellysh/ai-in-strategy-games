...
Вот пример нескольких настольных игр: крестики-нолики, шахматы, го. Все они имеют одинаковые характеристики с точки зрения теории игр:
...
В таких играх программе трудно учитывать все аспекты. Этих аспектов оказывается очень много. Поэтому сама программа должна быть достаточно сложной, чтобы охватить и возможные состояния игры, и возможные действия участников.
...
Теперь применим автоматический систематический поиск ошибок. Например, это может быть какой-то эволюционный алгоритм. Программа будет играть сама с собой и исправлять ошибки. Но во время такого обучения конями ходить она не будет из-за принципиального ограничения. В результате программа научится очень хорошо обращаться с теми фигурами, которыми она играла. Но будет ли такая программа хорошо играть в шахматы? Скорее всего, нет. Любой знающий правила игрок, сразу обнаружит ограничение программы и найдёт способ им воспользоваться.

### 3.6.3 Типы ошибок

До сих пор мы говорили об ошибках в шахматной программе, но без уточнения их видов. Понятие сложности игры позволяет нам разделить ошибки на три типа:
...
Самые сложные для обнаружения ошибки связаны с проектированием, программированием и алгоритмом обучения. Без анализа архитектуры и кода программы невозможно предсказать, в чём именно они заключаются и где их следует искать.

Для всех трёх типов ошибок применимы четыре стратегии поиска, которые мы рассмотрели.

### 3.6.4 Игра против современного шахматного движка
...
Сразу оговорюсь, что я ни в коем случае не преуменьшаю важность экспертных знаний профессиональных шахматистов. Кроме самых тривиальных случаев, когда машина банально не знает правил игры или не справляется с вычислительной сложностью, выиграть её может только человек с хорошими экспертными знаниями в игре. Чем эти выше уровень этих знаний, тем выше шанс победы.

Итак, рассуждения об ошибках привели нас к трём критериям для оценки шахматных программ:
...
Извлечь знания из обученной модели можно только косвенным способ. Например так, как это делает команда разработчиков Stockfish при обучении сети NNUE. Модель проходит по миллиардам шахматных позиций и даёт свою оценку для каждой из них. Эта оценка указывает на знания нейронной сети. К сожалению, человек не способен проанализировать такой объём информации, найти в ней закономерности и сделать выводы об ошибках целевого движка.
...
### 3.6.5 Общие выводы об игре против машины
...
Опять важная оговорка: чтобы играть с машиной, человек должен иметь соответствующие экспертные знания. Чем они выше, тем выше шансы человека на победу. Мы не рассматриваем случаи тривиальных ошибок у машины, которые сможет эксплуатировать игрок начального уровня.

Сценарии игры шахматиста против движка приводят нас к некоторой общей стратегии. Она состоит из следующих шагов:

1. Сбор информации о машине.

2. Создание повторяемой среды для поиска ошибок.

3. Поиск ошибок.

4. Эксплуатация найденной ошибки.

На первом шаге следует оценить возможности машины по трём критериям, о которых мы говорили:
...
Отрицательный ответ на второй вопрос даёт нам вид ошибок, который эксплуатировать несколько сложнее. Продолжим аналогию с шахматной программой. Допустим, что она работает на слабом компьютере. Его мощности хватает только для проверки дерева игры на четыре хода вперёд. Мы получаем эффект горизонта, которым несложно воспользоваться. Надо создать ловушку для программы на пятом ходу, которую она просто не увидит.

Если ответы на первые два вопроса положительные, нам придётся искать самый сложный вид ошибок машины. Ответ на третий вопрос даёт нам подсказку о том, как следует вести этот поиск.
...
Если стратегия окажется слабее, то скорее всего все потенциальные ошибки уже были найдены. При равной стратегии ещё есть шанс найти неисправленную ошибку. Почему? Потому что профессиональные шахматисты всё ещё играют друг с другом. Они используют равные стратегии поиска и при этом продолжают находить ошибки в игре друг с другом. Причём большинство из этих ошибок не "зевки". Более сильная стратегия поиска ошибок гарантированно даст положительный результат.

Последний шаг нашей стратегии — эксплуатация найденной ошибки.

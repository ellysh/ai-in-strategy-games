...
Второй пункт плана — создать полностью повторяемую среду для воспроизведения ошибки. Разработчики Deep Blue могли его выполнить. Они контролировали версии управляющей программы, шахматных чипов и их прошивки. Это позволяло воссоздать комбинацию аппаратного обеспечения и программы, в которой произошла проблема.
...
3. **Ошибки алгоритма поиска** — это изъяны реализации самого алгоритма и вспомогательных эвристик выборочного поиска. Они могут быть следующими:
   ...
   * Ошибка управления временем — программа неправильно учитывает время, отведённое на ход. Например, она агрессивно отсекает ветви дерева игры, когда в запасе остаётся ещё много времени. В этом случае программа принимает поспешное решение и не использует эффективно глубину поиска.
...
Теперь поговорим о противостоянии человека и машины в произвольной игре. Как и в случае компьютерных шахмат пусть против машины выступает не один игрок, а команда экспертов.

Шансы на победу команды экспертов зависят от ряда факторов. Основные из них следующие:

1. Правила и сложность игры.

2. Состав команды и уровень её участников.

3. Информация о машине, которая доступна экспертам. 

4. Ресурсы, которыми располагает команда. К ним относятся вычислительные мощности, время и специальные инструменты.

В зависимости от этих факторов эксперты более или менее качественно подготовятся к матчу с машиной. Информация является важнейшим аспектом. В общем случае чем больше эксперты знают или могут узнать о машине, тем эффективнее будет их подготовка.

План команды экспертов на подготовку и матч выглядит следующим образом:
...
4. Эксплуатировать ошибки машины в ходе матча.

Рассмотрим подробно каждый пункт.

**Первый шаг** — собрать информацию о машине. Его результат зависит от доступности информации и от состава команды экспертов. Если в команду входят разработчики аналогичных машин, они смогут эффективнее собирать сведения и использовать их.

Сбор информации можно разделить на две итерации. На **первой итерации** стоит выяснить самые важные факты. Среди прочего они подскажут, стоит ли вообще готовиться к матчу с машиной.

На первой итерации, команде экспертов надо выяснить следующее:

1. Какую стратегию поиска ошибок применяли разработчики машины?

2. Какой тип проблемной среды представляет собой игра? Речь идёт о типе с точки зрения теории интеллектуальных агентов.

3. Какой тип интеллектуального агента реализует машина?

Ответ на первый вопрос позволит оценить ресурсы, которые нужны команде экспертов. Чем сильнее стратегия поиска ошибок разработчиков машины, тем больше усилий и времени потребуется экспертам на подготовку. Возможно, у них просто не окажется достаточных ресурсов. Тогда лучше сразу отказаться от матча, потому что с плохой подготовкой победить сильную машину маловероятно.

Рассмотрим пример. Допустим, что разработчики машины искали ошибки с помощью стратегии АСП. При этом стало известно, что они располагают значительными вычислительными ресурсами (наподобие распределённой системы Fishtest). Если у команды экспертов нет сопоставимых вычислительных мощностей и подходящих инструментов для автоматического тестирования, им стоит перенести или отменить матч. Подготовка в таких условиях, скорее всего, закончится неудачей. Эксперты не найдут подходящих для эксплуатации слабостей машины.

Ответы на последние два вопроса укажут, есть ли у машины принципиальные проблемы. Эти проблемы возникают, когда сложность среды превышает возможности типа агента, который реализует машина. В этом случае у неё будут типичные изъяны. Некоторые из них мы рассмотрели в разделе 2.2.

Вот пример одного из типичных изъянов. Пусть игра представляет собой частично наблюдаемую среду. Машина реализует основанного на полезности агента, который следует некоторому алгоритму состязательного поиска. Что это за алгоритм, сейчас не важно. В таких условиях у машины есть принципиальная проблема с обработкой информации. Каждый ход игроки знают лишь часть фактов о текущем состоянии среды. По ходу партии доступная информация меняется. Человек способен объединить ранее полученные и новые факты, чтобы собрать полную картину происходящего. Но ни один алгоритм состязательного поиска на это не способен. Это означает, что машина также этого не может. Такой изъян даст человеку существенное преимущество.

Предположим, что команда экспертов выяснила тип проблемной среды, тип агента и стратегию поиска ошибок, которую применяли разработчики машины. Эта информация не указала на принципиальные проблемы машины. Но эксперты поняли, что шанс выиграть матч у них есть.
...
1. Точная версия машины, с которой будет проходить матч.

2. Исходный код её управляющей программы.

3. История изменений аппаратной и программной части машины.

4. Способ её реализации — один уникальный экземпляр или несколько.

5. Доступ к одному из экземпляров машины.

6. История прошлых партий машины.

Некоторые из этих пунктов могут остаться неизвестными. В худшем случае команде экспертов не удастся выяснить ни один из них. Но даже тогда у экспертов есть шанс успешно подготовиться к матчу.
...
Перед тем как искать ошибки машины, их стоит классифицировать по каким-то признакам. Классификация упорядочивает процесс поиска и позволяет эффективно разделить задачи между специалистами разного профиля, которые входят в команду экспертов.

В случае компьютерных шахмат мы классифицировали ошибки по компоненту программы, в котором есть изъян. Это не единственно возможный вариант. Признаками также могут быть аспекты игры, в которых машина ошибается. В общем случае классификация зависит от свойств конкретной игры и особенностей машины.

Отправной точкой для классификации ошибок могут стать типичные изъяны программ для настольных игр. Эти изъяны следующие:

1. **Машина не справляется со сложностью правил игры**. Например, в игре так много объектов, их параметров и способов взаимодействия, что машина не способна учесть все возможные комбинации. В этом случае достаточно найти такие состояния среды, которые машина не понимает. Её действия в этих состояниях будут неадекватными.

2. **Машина не справляется с вычислительной сложностью игры**. Пример такой проблемы — эффект горизонта в ранних шахматных программах. Они просчитывают позиции до четырёх ходов вперёд. Это означает, что программы не видят планы и комбинации оппонента дальше чем на четыре хода.

3. **Логические ошибки при выборе действия**. Рассмотрим пример. Пусть машина или её часть реализована как рефлексный агент. В этом случае типична ошибка интерпретации текущего состояния среды. Если два состояния похожи по некоторым признакам, машина может их перепутать. Тогда она выберет неверное действие.

4. **Ошибки обучения модели**. Если машина реализует обучающегося агента, стоит проверить типичные ошибки алгоритма обучения: низкая производительность модели, переобучение, недообучение, плохая обобщающая способность и т.д.

5. **Ошибки долгосрочного планирования**. Такие проблемы встречаются у всех типов интеллектуальных агентов. В сложных играх решения участников могут иметь долгосрочные последствия. Если машина не может связать причины и следствия игровых событий, её действия будут неадекватными.

Это не универсальная классификация. Она достаточно проста и предлагает пять типов ошибок, которые характерны для машин с разной реализацией. В некоторых случаях её будет достаточно.

Мы уже рассмотрели три сценария игры команды экспертов против шахматной программы. Они универсальны и подходят к любой игре. Рассмотрим условия для выполнения каждого сценария.

**Первый сценарий РНП** команда экспертов применяет в следующем случае:

1. Команда состоит только из игроков.

2. На второй итерации сбора информации экспертам не удалось ничего выяснить.

3. Доступа к экземпляру машины нет.

4. У экспертов нет специальных инструментов.

5. Известно, что разработчики машины применяли стратегию поиска ошибок не сильнее РНП.

Если последнее условие не выполняется, экспертам стоит отказаться от матча. Как мы уже выяснили, в этом случае их шансы на победу малы.

Команда экспертов применяет **второй сценарий РСП** в следующем случае:

1. В команду входят игроки и специалисты по аналогичным машинам.

2. На второй итерации сбора информации экспертам удалось выяснить большинство пунктов.

3. Есть доступ к экземпляру машины.

4. У экспертов нет специальных инструментов.

5. Известно, что разработчики машины применяли стратегию поиска ошибок не сильнее РСП.

**Третий сценарий АСП** команда экспертов применяет в следующем случае:

1. В команду входят игроки, специалисты по аналогичным машинам, тестированию и статистике.

2. На второй итерации сбора информации экспертам удалось выяснить большинство пунктов.

3. Есть доступ к экземпляру машины.

4. У экспертов есть специальные инструменты.

5. Разработчики машины применяли стратегию поиска ошибок не сильнее АСП. Если они применяли АСП, то их ресурсы сопоставимы с ресурсами команды экспертов.

Допустим, что эксперты выбрали один из трёх сценариев и все его условия выполняются. В этом случае у них есть хорошие шансы на победу. Чем сильнее стратегия поиска ошибок команды экспертов, тем выше эти шансы.

**Третий шаг** плана — научиться воспроизводить найденные ошибки машины. Успех этого шага зависит от доступа к экземпляру машины, обмена информацией внутри команды экспертов, уровня игроков и ресурсов, которыми они располагают.

Если у команды экспертов нет доступа к экземпляру машины, они не смогут проверить обнаруженные ошибки. Игрок команды получит такую возможность только во время матча. В этом случае третий шаг плана невыполним.

Проверка ошибок начинается с того, что специалисты по разработке машин, тестированию и статистике объясняют игрокам команды свои находки. Игроки, в свою очередь, должны понять суть найденных проблем. Опираясь на свой опыт, они соотносят изъяны программы и конкретные игровые ситуации. Затем игроки воспроизводят эти ситуации и проверяют, как ведёт себя машина.

Уровень игроков команды экспертов важен. Чем он выше, тем разнообразнее ошибки машины, которые они могут воспроизводить и эксплуатировать. Начинающий игрок способен понять грубую ошибку оппонента и воспользоваться ей. Но только профессионал сможет заметить и извлечь выгоду из незначительного стратегического просчёта противника.

Команда экспертов располагает какими-то ресурсами. Их дефицит значительно осложняет воспроизведение ошибок машины. Рассмотрим пример. Допустим, что машина отыгрывает только полные партии от начала до конца. Игра сложная и одна партия занимает много времени. Предположим, что команда экспертов нашла десять потенциальных проблем в поведении машины. Чтобы воспроизвести и проверить их все, игрокам команды нужно значительное время. Если до начала матча этого времени нет, им придётся выбрать несколько самых перспективных ошибок и работать только с ними. Это уменьшает эффективность подготовки команды.

**Четвёртый шаг** плана — эксплуатировать ошибки машины в ходе матча. На этом шаге важнейший фактор — это уровень игрока команды экспертов, который участвует в партиях. Он во многом определит исход противостояния.

Уровень игрока определяет следующее:

* Как часто будет ошибаться игрок?

* Сможет ли он создать игровые ситуации, в которых воспроизводятся найденные слабости машины?

* Сможет ли игрок извлечь преимущество из ошибок машины?

* Хватит ли полученного преимущества для победы?

Если уровень машины превышает человеческий, любая ошибка игрока может стоить поражения в партии. На таких условиях соревноваться с машиной способен только чемпион мира или претендент.

Во время матча также важно вмешательство разработчиков машины. Они могут вносить исправления между партиями. В этом случае у команды экспертов должен быть достаточный набор ошибок для эксплуатации. Если известных ошибок мало, разработчики могут исправить их все. Тогда команда экспертов потеряет всё преимущество, которое дала им подготовка к матчу.

Независимо от вмешательства разработчиков в работу машины, экспертам нужно спланировать матч. Они должны решить, в каких именно партиях какую уязвимость машины им выгоднее использовать. Это зависит от правил игры, регламента соревнования и текущих результатов.

Мы рассмотрели общий план команды экспертов на матч против машины. Он не гарантирует победы, особенно если машина играет на сверхчеловеческом уровне. Хорошая подготовка экспертов только повышает их шансы на успех. При этом остаётся место для случайностей, человеческого фактора и ошибок игрока, который соревнуется с машиной. Всё это делает исход матча трудно предсказуемым.

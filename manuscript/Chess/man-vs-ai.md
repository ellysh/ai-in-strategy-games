...
Сбор информации можно разделить на несколько итераций. На **первой итерации** стоит собрать критически важные факты. Среди прочего они подскажут, стоит ли вообще готовиться к матчу с машиной.

На первой итерации, команде экспертов надо выяснить следующее:

1. Какую стратегию поиска ошибок применяли разработчики?

2. Какой тип проблемной среды представляет из себя игра? Речь идёт о типе с точки зрения теории интеллектуальных агентов.

3. Какой тип интеллектуального агента реализует машина?

Ответ на первый вопрос позволит оценить ресурсы, которые нужны команде экспертов для подготовки к матчу. Чем сильнее стратегия поиска ошибок разработчиков, тем больше усилий и времени потребуется экспертам. В некоторых случаях, у них может просто не оказаться достаточных ресурсов. Тогда лучше сразу отказаться от матча, потому что победа над машиной в таких условиях маловероятна.
...
Ответы на последние два вопроса укажут на потенциальные слабости машины. Для примера допустим, что сложность среды превышает возможности типа агента, который реализует машина. В этом случае у машины будут типичные изъяны, которые мы рассмотрели в разделе 2.2.

Вот пример типичного изъяна. Пусть игра представляет собой частично наблюдаемую среду. Машина реализует основанного на полезности агента, который следует некоторому алгоритму состязательного поиска. Что это за алгоритм, сейчас не важно. В таких условиях у машины есть принципиальная проблема с обработкой информации. Каждый ход игроки знают лишь часть фактов о текущем состоянии среды. По ходу партии доступная информация меняется. Человек способен объединить ранее полученные и новые факты, чтобы получить полную картину. Но ни один алгоритм состязательного поиска на это не способен. Это означает, что машина также этого не может. Кроме того, она не учитывает информацию, которая доступна оппоненту. Эти изъяны можно использовать, чтобы получить преимущество.

Предположим, что команда экспертов выяснила тип проблемной среды, тип агента и стратегию поиска ошибок, которую применяли разработчики машины. Эта информация не дала подсказок о вероятных изъянах машины. Но эксперты поняли, что шанс выиграть матч у них есть.
...
Некоторые из этих пунктов могут остаться неизвестными. В худшем случае команде экспертов ни удастся выяснить ни один из них. Даже тогда у них будет шанс успешно подготовиться к матчу.
...
В случае компьютерных шахмат мы классифицировали ошибки по компоненту программы, в котором есть изъян. Это не единственно возможный вариант. Например, признаками могут быть аспекты игры, в которых машина ошибается.
...
2. **Машина не справляется с вычислительной сложностью игры**. Пример такой проблемы — эффект горизонта в ранних шахматных программах. Они просчитывали позиции до четырёх ходов вперёд. Это означает, что программа не видит планы и комбинации оппонента на пять и более ходов вперёд.
...
3. У экспертов есть специальные инструменты.

4. Разработчики машины применяли стратегию поиска ошибок не сильнее АСП. Если они применяли АСП, то их ресурсы сопоставимы с ресурсами команды экспертов.

Если выполняются условия каждого из трёх сценариев и эксперты применяют соответствующую стратегию поиска ошибок, у них есть хорошие шансы на победу в матче с машиной. 

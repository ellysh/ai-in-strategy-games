...
Во время шахматной партии участники могут запоминать или записывать все сыгранные ходы. Следовательно, каждый из них знает все игровые события на момент своего хода. Стратегической неопределённости нет, поэтому шахматы — это игра с совершенной информацией.
...
* **Классический**. На первые 40 ходов каждому игроку отводится 90 минут. После 40-го хода каждому игроку добавляется 30 минут. К этим 30 минутам добавляется по 30 секунд за каждый ход, сделанный с начала партии.
...
* **Армагеддон** (armageddon). Игрок за чёрные фигуры выигрывает в случае ничьей. Ему отводится на 1 минуту меньше времени, чем игроку за белые фигуры. Белые имеют 6 или 5 минут на все ходы. Соответственно, чёрные имеют 5 или 4 минуты.
...
После двух ходов игроков мы получили `20 * 20 = 400` узлов действия. На третьем ходу у белых есть 26 возможных ходов. На каждый из них чёрные отвечают своими 26-ю ходами. Поэтому к четвёртому ходу дерево игры имеет в сумме `20 * 20 * 26 * 26 = 270400` узлов действия.
...
5. **Сложность реализации**. Эвристику выбора перспективных ходов реализовать сложнее, чем функцию оценки. Поэтому в ней часто встречаются ошибки, которые сильно снижают качество отбора ходов.
...
Мы познакомились с одной из самых значительных работ по применению теории игр к шахматам. Она указала направление для разработки эффективных алгоритмов поиска ходов. Кроме этого, метод обратной индукции стали применять специальные программы для просчёта эндшпилей. Они находят лучшие ходы для мата с заданными наборами фигур. Отнесём эти примеры к вкладу теории игр в шахматную теорию. Теперь поговорим о её практическом вкладе в шахматную игру.

В реальных турнирных партиях шахматисты никогда не применяют математический аппарат теории игр. У него есть несколько проблем, о которых мы говорили в заключении первой главы. Здесь мы продолжим этот список, но уже применительно к задачам шахматной игры.

Во-первых, точные алгоритмы теории игр требуют большого объёма вычислений. Человек тратит слишком много времени, чтобы применить алгоритм минимакс и его производные (например, альфа-бета отсечение) в сложной шахматной позиции.

Во-вторых, оценка шахматной позиции — это сложный процесс. Кроме материального баланса надо учитывать такие факторы как активность фигур, пешечная структура, контроль ключевых полей и т.д. Чтобы учесть эти факторы в математическом аппарате теории игр, их надо перевести в численные показатели. В результате построенная модель игры оказывается очень сложной.

В-третьих, игра в шахматы имеет динамический характер. Ценность определённых фигур и позиций резко меняется в ходе игры. Это ещё больше усложняет модель и затрудняет применение алгоритмов теории игр человеком.

В-четвёртых, игра в шахматы на высоком уровне имеет психологические аспекты. Она включает такие элементы, как психологическая борьба, блеф, игру на эмоциях и ожиданиях противника. Эти аспекты трудно измерить и учесть в рамках теории игр.

В-пятых, опытные игроки часто полагаются на интуицию. Особенно она помогает в сложных позициях, когда просчёт всех вариантов невозможен. Теория игр полностью игнорирует этот естественный для человека способ принятия решений. Она полагается только на точные расчёты и рациональность.

Из-за этих трудностей шахматисты не могут эффективно применять теорию игр в своих партиях. Шахматным программам удаётся скомпенсировать некоторые из перечисленных проблем, благодаря высокой вычислительной мощности компьютеров. Они хорошо справляются со сложностью модели и большим объёмом вычислений. Этого оказалось достаточно для сильной игры.

Мы пришли к выводу, что на методах теории игр строится большинство шахматных программам. С другой стороны, её вклад в профессиональные шахматы оказался очень ограниченным.

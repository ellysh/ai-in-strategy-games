...
В августе 1969 года Кен Томпсон и Деннис Ритчи из Bell Labs разработали первый прототип операционной системы Unix для миникомпьютера PDP-7. Томпсон был любителем шахмат и решил написать пробную шахматную программу [Belle](https://www.chessprogramming.org/Belle, работающую на новой ОС.
...
11 мая 1997 года шахматный компьютер [Deep Blue](https://ru.wikipedia.org/wiki/Deep_Blue) победил действующего чемпиона мира по шахматам Гарри Каспарова. История разработки этого компьютера длилась более 10 лет. Рассмотрим, как небольшой группе исследователей это удалось.
...
С середины 1980-х в университете Карнеги Мелоун группа студентов и преподавателей разрабатывала шахматный компьютер [HiTech](https://www.chessprogramming.org/HiTech). Руководил проектом заслуженный эксперт по шахматным программам профессор [Ханс Берлинер](https://www.chessprogramming.org/Hans_Berliner).
...
Для разработки аппаратной части HiTech Берлинер привлекал студентов. В 1985 году профессор обратился к [Фэн Сюн Сю](https://www.chessprogramming.org/Feng-hsiung_Hsu), который тремя годами ранее переехал в США из Тайваня и поступил на кафедру информатики (computer science) в университет Карнеги Мелоун.
...
Опираясь на свой опыт, Фэн Сюн Сю предложил полностью изменить архитектуру HiTech. Его идея была в том, чтобы заменить стандартные чипы на специализированные. Тогда весь генератор ходов можно было разместить на одном СБИС чипе вместо 64-х. Далее Фэн Сюн Сю продумал оптимизацию модуля оценочной функции. Это позволило поместить его в тот же чип, что и генератор ходов.

Переход к системе на одном чипе теоретически увеличивал производительность шахматного компьютера на порядок. Однако, Берлинер отклонил предложение Фэн Сюн Сю. По его мнению основная игровая сила HiTech заключалась в алгоритме B*, который должен был выполнять модуль контроля поиска. Поэтому профессор стремился как можно скорее закончить разработку генератора ходов и модуля оценки позиций. После этого он смог бы заняться интересующим его модулем контроля поиска.
...
2. Использовать большое количество шахматных машин параллельно, чтобы ускорит поиск.

В 1986 году Фэн Сюн Сю получил первые чипы, изготовленные по его схемам. Эту работу выполнила компания по производству полупроводников, которая сотрудничала с американскими университетами.

Фэн Сюн Сю написал простую программу для тестирования чипов на брак производства. Но такого теста было недостаточно для проверки их производительности. Сю хотел, чтобы полноценная шахматная программа запустилась на его чипе и сыграла несколько партий.

Томас Анантараман учился на той же кафедре информатики, что Фэн Сюн Сю. У Анантарамана уже была готовая шахматная программа для компьютера общего назначения. Он написал её для развлечения и не имел планов на её развитие. Анантараман согласился помочь Сю с тестированием чипов. Он установил чипы в свой компьютер и адаптировал для них шахматную программу. В результате им удалось достичь скорости поиска около 30000 позиций в секунду. Такую производительность в 1986 году имели только специализированные шахматные компьютеры или шахматные программы, запущенные на суперкомпьютерах.
...
В это время к проекту присоединился аспирант Мюррей Кэмпбелл. Он был сильнейшим шахматистом среди аспирантов и помогал программировать оценочную функцию.
...
Уже на следующем чемпионате ACM 1987 года скорость поиска ChipTest достигла 400000 позиций в секунду. Это стало возможным благодаря новой рабочей станции. Она заменила устаревшую Perq.

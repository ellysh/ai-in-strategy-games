...
Исследование показало, что командиры пожарных бригад не оценивают план действий по каким-то критериям. Вместо этого они представляют себе выполнение плана шаг за шагом. То есть в ходе единичной оценки специалист мысленно выполняет действия. Если на каком-то шаге возникает проблема, эксперт либо ищет её решение, либо отбрасывает план и переходит к следующему. Этот процесс Гэри Кляйн назвал **ментальной симуляцией** (mental simulation). Учёный объяснил эту концепцию так:

> Ментальная симуляция — это способность осознанно воображать людей и предметы и преобразовывать их за несколько переходов, чтобы в конечном счёте они составили иную картину, отличную от начальной.
...
Следующие исследования Гэри Кляйн провёл с шахматными гроссмейстерами. Оказалось, что модель RPD хорошо объясняет, как они выбирают ходы.

Эксперимент показал, что при выборе наилучшего хода гроссмейстеры полагаются на свой опыт. Прежде всего они распознают прототип текущей позиции на доске. Это даёт шахматисту несколько перспективных ходов для найденного прототипа. Для каждого из них игрок проводит ментальную симуляцию. Просчитав варианты на несколько ходов вперёд, гроссмейстер получает новую возможную позицию. Чтобы её оценить, опытный игрок не использует общие критерии шахматной теории (например, пешечная структура или активность фигур). Вместо этого он снова распознаёт прототип позиции, полагаясь на свой опыт. Обнаруженный прототип вызывает определённую эмоциональную реакцию. Это может быть энтузиазм или разочарование. Именно такая эмоциональная оценка и служит критерием для быстрого выбора наилучшего хода.

Гэри Кляйн выяснил, что опытные шахматисты распознают не только прототипы позиций на доске, но и **точки воздействия**. Учёный так определяет это понятие:

> Точка воздействия — это фокусная точка построения решения. Отправной момент для изобретательного решения проблемы.
...
Шахматист распознаёт точки воздействия, полагаясь на свой опыт. Этот процесс напоминает распознавание прототипа позиции на доске. Но в отличие от него, точка воздействия не предлагает готового плана действий. Она только указывает на ключевую идею или фрагмент цепочки действий. Эксперт использует этот материал как отправную точку для разработки плана.
...
Учёный и его коллеги организовали два турнира: один для трёх мастеров и один для трёх игроков слабого уровня (рейтинг Эло 1600-1800). В каждом турнире шахматисты должны были играть партии по классическим правилам (90 минут на все ходы) и блиц (4 минуты). Исследователей консультировала команда гроссмейстеров. Они оценивали качество ходов в каждой партии турнира. Чтобы сократить объём работы, рассматривались только ходы в сложных позициях.
...
Кроме качества ходов приглашённые гроссмейстеры оценивали количество ляпов. **Ляпом** в шахматах называется очень плохой ход. У мастеров доля ляпов составляла 7% в блиц партиях и 8% в классических. У слабых игроков отношение было такое: 25% в блиц и 11% в классических.

Гэри Кляйн пришел к выводу, что дефицит времени практически не влияет на качество решений эксперта. Для новичка дефицит времени оказывается критическим ограничением: качество его решений значительно падает. Учёный объяснил этот результат следующим образом. Новичку в отличие от эксперта недостаёт опыта. Поэтому он не может подобрать знакомый прототип для большинства позиций, с которыми сталкивается в ходе партии. Поэтому он чаще вынужден следовать стратегии сравнительной оценки, которая требует недопустимо много времени для условий блиц партии.

### 3.3.8 Профессиональная игра

Мы рассмотрели ряд важных идей и моделей когнитивной психологии. Все вместе они дают общее представление о том, как именно профессиональный шахматист принимает решения во время игры.

Когнитивная архитектура CHREST объясняет, как шахматист обучается и достигает уровня эксперта. В основе этого обучения лежит запоминание шаблонов позиций на доске и тренировка в их распознавании.

Правило 10000 часов даёт представление о том, как профессионалы строят эффективный процесс обучения. Для этого они выполняют регулярную преднамеренную практику под руководством тренера.

Модель RDP описывает как профессиональный шахматист принимает решения непосредственно во время партии. Он полагается на распознавание знакомых ему типичных шаблонов. Каждому шаблону соответствуют действия, которые хорошо зарекомендовали себя в прошлом. Шахматист выполняет ментальную симуляцию. Так он подтверждает для себя, что эти действия подойдут для текущей позиции на доске.

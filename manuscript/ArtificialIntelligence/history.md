...
Перед работой перцептрон необходимо обучить. После этого он сможет определять к какому классу относится предъявленный ему объект. Розенблатт использовал идеи Хебба и разработал два метода обучения перцептрона.
...
В 1962 году Розенблатт опубликовал книгу "Принципы нейродинамики" (Principles of Neurodynamics). В ней он рассматривает различные архитектуры перцептронов, в том числе и многослойные. Также в книге Розенблатт доказал [теорему сходимости перцептрона](https://ru.wikipedia.org/wiki/Теорема_сходимости_перцептрона). Согласно теореме, перцептрон можно обучить за конечное число шагов. После такого обучения он сможет решить поставленную задачу.

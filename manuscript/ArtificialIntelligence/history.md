...
| --- | --- | --- |
| [Символьный](https://ru.wikipedia.org/wiki/Символический_искусственный_интеллект) | Задача | Аллен Ньюэлл |
| подход | решается через действия над понятными человеку символическими обозначениями. | Герберт Саймон |
|  | | |
...
Исследования Аллена Ньюэлла и Герберта Саймона заложили основу **символьного подхода** (symbolic approach). Его идея заключается в создании интеллектуальных систем, которые манипулируют символами. Символы могут быть абстрактными, но должны быть понятны человеку. Саму систему надо программировать вручную, как и любую другую компьютерную программу.
...
I> **Формальная логика** — это раздел логики, в котором применяется формальный язык и строгие правила для операций над высказываниями. **Математическая логика** — это направление формальной логики, в котором активно применяется математический аппарат.
...
Эти события привели к крупным успехам интеллектуальных систем в двух областях: распознавание образов и обработка естественного языка.
...
* Обучение deep learning моделей требует значительных вычислительных ресурсов и специализированного оборудования (например, GPU). Даже используя большие вычислительные мощности, алгоритм обучения может выполняться несколько дней или недель.

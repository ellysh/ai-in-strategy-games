...
Проблема заключалась в том, что системе приходилось проверять много вариантов картин масс-спектра. Разработчики Dendral обсудили это затруднение с химиками. Вместе они пришли к идее, как сократить число потенциальных решений. Для этого в систему добавили знания о типичных картинах пиков в масс-спектре вещества. Эти пики указывают на наличие определённых фрагментов молекул.

Дополнительное правило, которое ограничивает число возможных решений, получили названия [эвристика](https://ru.wikipedia.org/wiki/Эвристика). Dendral стала первой ИИ системой, которая применяет эвристики на основе знаний из предметной области. Этот подход позволил системе успешно справляться со своей задачей.
...
В 1978 году Джон Мак-Дермотт из университета из университета Карнеги — Меллона разработал первую коммерческую экспертную систему [XCON](https://en.wikipedia.org/wiki/Xcon). Заказчиком системы была компания DEC, которая производила компьютера. Система XCON помогала сотрудникам DEC составлять конфигурации компьютеров, согласно требованиям заказчиков. Экспертная система экономила компании до 40 миллионов долларов в год.

Громкий успех системы XCON привел к тому, что корпорации по всему миру начали разработку экспертных систем для внутреннего использования. В эти разработки включились производители компьютерного оборудования. Начали появляться специальные компьютеры под названием [Lisp-машины](https://ru.wikipedia.org/wiki/Лисп-машина). Они предназначались для запуска программ на языке [Lisp](https://ru.wikipedia.org/wiki/Лисп). Этот язык стал основным для разработки экспертных систем.

#### Компьютеры пятого поколения

В 1981 году правительство Японии выделило 850 миллионов долларов на создание [компьютера пятого поколения](https://ru.wikipedia.org/wiki/Компьютеры_пятого_поколения). Этот проект предполагал создание производительного суперкомпьютера, основанного на [параллельных вычислениях](https://ru.wikipedia.org/wiki/Параллельные_вычисления). Параллельность достигалась за счёт использования нескольких процессоров. Данные для вычислений предполагалось хранить не в файловой системе, а в базе данных.

Кроме того компьютер должен был обладать следующими функциями ИИ:
...
Другие страны ответили на амбициозные планы Японии. Англия начала собственный проект по созданию компьютера пятого поколения под названием [Alvey](https://en.wikipedia.org/wiki/Alvey). Группа американских компаний объединилась и создала [корпорацию MCC](https://en.wikipedia.org/wiki/Microelectronics_and_Computer_Technology_Corporation), также известную как Консорциум Микроэлектрони и Компьютеров. Эта корпорация должна была спонсировать фундаментальные исследования в области ИИ и информационных технологий. Агенство министерства обороны США DARPA утроило финансирование в области ИИ.
...
Однако, в 1987 году рынок Lisp-машин неожиданно рухнул. Причиной этого стало появление персональных компьютеров (ПК) от компаний Sun Microsystems, Apple и IBM. По сравнению с Lisp-машинами ПК были значительно дешевле и производительнее.

Единственное, что останаваливало пользователей экспертных систем от перехода на ПК, была поддержка языка Lisp. Но эта проблема скоро решилась. Компании Lucid и Franz LISP предложили интерпретатор языка Lisp для компьютеров от Sun Microsystems под управлением операционной системы Unix. Позднее появились интерпретаторы Lisp для компьютеров от Apple и IBM. В результате ПК смогли исполнять Lisp-программы быстрее, чем Lisp-машины. Пользователям не было никакого смысла платить за специализированные компьютеры.

...
Направление deep learning также известно как [**глубокое обучение**](https://ru.wikipedia.org/wiki/Глубокое_обучение). Это один из подходов машинного обучения. Отличительная особенность deep learning в архитектуре нейронных сетей. Вместо трёх слоев, как в перцептроне Розенблатта, эти сети имеют больше промежуточных слоев. Благодаря им, алгоритм обучения может извлекать особенности разного уровня асбтракции из входных данных. Например, алгоритм машинного зрения может различать на низком уровне отдельные линии, а на более высоком цифры и буквы.

Подход deep learning решил некоторые проблемы традиционных моделей нейронных сетей. В результате удалось достич такой же точности распознования образов, как и у человека в нескольких узких областях. Например, в распознавании знаков дорожного движения и рукописного текста. Также удалось значительно повысить качество систем распознования речи.

#### Big Data

Другое современное направление в ИИ называется big data или [**большие данные**](https://ru.wikipedia.org/wiki/Большие_данные). Оно рассматривает методы обработки наборов данных, которые слишком велики для применения традиционных подходов.

Big data занимается следющими задачами:

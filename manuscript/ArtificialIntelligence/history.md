...
В середине 1960-х годов исследователи из Стэнфордского университета предложили новый подход к разработке символьных ИИ систем. Он получили название [**экспертные системы**](https://en.wikipedia.org/wiki/Expert_system).

И экспертные, и более ранние символьные системы относятся к типу [**основанных на знаниях**](https://en.wikipedia.org/wiki/Knowledge-based_systems) (knowledge-based system или KBS). Их отличительная особенность — это архитектура. Система состоит из двух частей: **базы знаний** и **механизма вывода**. Впервые такую архитектуру предложил Джон Маккарти для своей гипотетической системы Advice Taker.

База знаний содержит факты о реальном мире или прикладной области. Эти факты представлены в специальной форме, а не в виде программного кода. Например, это могут быть правила записанные на [формальном языке](https://ru.wikipedia.org/wiki/Формальный_язык) или в виде логических выражений.

Механизм вывода обрабатывает информацию из базы знаний, чтобы из неё получить новые знания. Этот механизм строится на алгоритмах и эвристиках поиска.

Экспертная система отличается от более ранних систем, основанных на знаниях. Её особенность в узкой специализации. Она имитирует процесс принятия решений человеком-экспертом в одной конкретной прикладной области. Для этого система использует знания, которые человек-эксперт предварительно заносит в её базу знаний. Эти знания он формулирует на основе своего образования и профессионального опыта.

Почему экспертные системы решили проблему комбинаторного взрыва? Ранние ИИ системы оперировали достаточно общими фактами. На первом этапе работы система из фактов выводила промежуточные заключения, которые относились непосредственно к задаче. На втором этапе она переходила от промежуточных выводов к конечному решению. Комбинаторный взрыв происходил на первом этапе. Исходных фактов было слишком много и построенное по ним дерево поиска было невозможно обработать.

Экспертные системы просто исключают стадию расчёта промежуточных выводов. Они оперируют не общими элементарными фактами, а знаниями о предметной области. Эти знания представляют собой уже готовые промежуточные заключения, из которых сразу можно прийти к решению задачи. Поэтому экспертные системы выполняют значительно меньше вычислений.

Экспертные системы стали новым шагом в развитии символьного подхода. Их главная идея в узкой специализации. Каждая система разрабатывалась для решения одной конкретной прикладной задачи. Такой подход позволил увеличить уровень абстракции 

>>>

----

 База знаний этих систем представляла собой набор символов. Причем [**уровень абстракции**](https://dou.ua/lenta/articles/level-of-abstraction/) этих символов стал значительно выше чем у ранних систем ИИ.

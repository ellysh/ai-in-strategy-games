...
Первая версия перцептрона была реализованна в виде программы для компьютера [IBM 704](https://ru.wikipedia.org/wiki/IBM_704). Перцептрон представляет собой искусственную нейронную сеть с особенной архитектурой. Сеть состоит трёх слоёв нейронов:

1. S-элементы (сенсорные)
2. A-элементы (ассоциативные)
3. R-элементы (реагирующие)

Чтобы лучше понять структуру перцептрона, проведём аналогию с мозгом. Перцептрон создаёт набор ассоциаций (A-элементы) между входными стимулами (S-элементы) и реакцией на них (R-элементы). Пример такой реакции — физиологический ответ двигательных нейронов на зрительную информацию.
...
Big data или [**большие данные**](https://ru.wikipedia.org/wiki/Большие_данные) — это одно из современных направлений в ИИ. Это направление вознкло из-за [компьютеризации](https://ru.wikipedia.org/wiki/Компьютеризация) общества. Все больше компаний и правительственных учреждений используют компьютеры. В результате накапливаются огромные объёмы информации. Их надо как-то обрабатывать, но после определённого порога традиционные методы перестают работать.

Направление big data изучает методы обработки наборов данных, которые слишком велики для применения традиционных подходов.

Исследователи big data занимается следющими задачами:
...
В 1986-ом году в машинном обучении появилось новое направление под названием deep learning. Оно также известно как [**глубокое обучение**](https://ru.wikipedia.org/wiki/Глубокое_обучение). Отличительная особенность этого подхода в архитектуре нейронных сетей. Эти сети называются **глубокими**. В отличие от перцептрона Розенблатта они имеют более трёх слоёв. Благодаря им, алгоритм обучения может извлекать признаки разного уровня асбтракции из входных данных. Например, алгоритм машинного зрения может различать на низком уровне отдельные линии, а на более высоком цифры и буквы.

Активные исследования по изучению многослойных сетей начались в середине 1960-х годов. В 1967 году советский математик [Алексей Ивахненко](https://ru.wikipedia.org/wiki/Ивахненко,_Алексей_Григорьевич) предложил первый алгоритм обучения многослойных перцептронов.

Исследования многослойных нейронных сетей продолжались на протяжении 1970-х и 1980-х годов. Однако, только в 2000-х годах глубокие сети стали широко использоваться. Причин этому несколько:

* Возросла общая производительность компьютеров.
* Были накоплены большие наборы данных для обучения сетей для задач в различных предметных областях.
...
Подход deep learning решил некоторые проблемы традиционных моделей нейронных сетей. В результате удалось достич такой же точности распознования, как и у человека в нескольких узких областях. Например, в распознавании знаков дорожного движения и рукописного текста. Также удалось значительно повысить качество систем распознования речи.
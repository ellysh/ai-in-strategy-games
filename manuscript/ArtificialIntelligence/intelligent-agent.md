...
Рациональные агенты нужны для решения какой-то конкретной задачи. Эту задачу описывает **проблемная среда** (task environment).

Проблемная среда включает в себя следующее:
...
На практике возможно много вариантов проблемных сред. Программы агентов для похожих сред будут работать по одинаковому принципу. Все возможные среды можно классифицировать по некоторым признакам. Тогда при разработке нового агента легче сформулировать требования. Тип среды, в которой он должен работать, даст подсказку.
...
Рассмотрим подробнее каждый тип проблемной среды.

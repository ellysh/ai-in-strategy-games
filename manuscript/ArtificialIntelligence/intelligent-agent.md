...
Все рефлексные агенты работают по таблице правил. Эта таблица представляет собой код с условными операторами или структуру данных в памяти компьютера. **Агент, основанный на цели** использует принципиально другой способ принятия решений. Ему известно состояние среды, которое нужно достичь.

Рассмотрим пример работы агента. Предположим, что он действует в неизвестной среде. Агент знает свою цель. Поэтому после каждого действия он может проверить — достигнуто ли нужное ему состояние? Такое поведение называется **поиском в оперативном режиме**. Поиск позволяет агенту работать в неизвестной среде.

Агент, основанный на цели, может работать только в **безопасно исследуемых** неизвестных средах. Это свойство означает, что целевое состояние среды достижимо из любого другого достижимого состояния. Другими словами, в среде нет тупиков, из которых агент не сможет выбраться.

Если среда известна агенту, он может заранее проверить результаты всех своих действий. Когда какая-то последовательность действий приводит к цели, агент её выполняет. Таким образом решение задачи снова сводится к поиску.

Для перебора всех возможных действий агента и их результатов используются **алгоритмы поиска**. Для удобства действия агента и их результаты представляются в виде дерева или графа. Каждый узел дерева — это определённое состояние среды. К этому состоянию приводят действия агента, которые обозначаются входящим в узел ветвям. Исходящие из узла ветви — это возможные действия агента в данном состоянии среды.

Алгоритм поиска обходит дерево состояний среды и действий агента. Известная агенту цель соответствует определённому состоянию среды. Когда алгоритм поиска находит узел с таким состоянием, задача считается решённой. Агенту остаётся только выполнить последовательность действий, которая ведёт из начального узла дерева к найденному.
...
Возможности основанного на цели агента ограничивает память и вычислительная мощность компьютера. Алгоритмы поиска сохраняют в памяти информацию о ранее посещённых узлах. Для некоторых алгоритмов объем памяти зависит экспоненциально от количества посещённых узлов. Другие алгоритмы хранят меньше информации, но за это многократно проходят по одним и тем же узлам. Самые эффективные алгоритмы поиска находят компромисс между объёмом использованной памяти и временем работы.

Агент, основанный на цели, не способен действовать в мультиагентной среде. Он не знает какие из его действий и действий других агентов приближают его к цели, а какие удаляют. Но в мультиагентной среде агенты могут мешать друг другу. Использующий поиск агент не сможет правильно учесть действия других агентов. Поэтому достигнуть цели за разумное время он не сможет.

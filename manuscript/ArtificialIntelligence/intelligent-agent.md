...
Функцию агента можно представить в виде **таблицы правил**. Эта таблица задаёт соответствия между каждой возможной последовательностью актов восприятия и действием агента.
...
Решение задачи очистки диска с помощью таблицы правил неоптимально. Представьте, что наш агент должен проработать не один день, а неделю. Тогда размер его программы вырастет до `466 * 7 = 3262` мегабайт.
...
Чтобы наш агент проработал один день, ему больше не нужна таблица размером 466 мегабайт. Вместо этого ему достаточно таблицы 2-7, которая занимает в памяти компьютера 4 бита. Её размер не зависит от времени работы агента.

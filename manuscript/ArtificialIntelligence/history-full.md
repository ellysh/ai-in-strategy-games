...
* x~i~ — значение i-го входного сигнала
...
* [**Эвристика**](https://ru.wikipedia.org/wiki/Эвристика) — это дополнительное правило, которое исключает из рассмотрения некоторые ветви дерева состояний. Эвристика позволяет предсказать без проверки, что какая-то ветвь не приведёт к решению. Удаление таких ветвей значительно сокращает время поиска.
...
Такие системы хорошо справляются с небольшими формальными задачами. Например, с теоремами из книги "Начала математики". Если построить дерево с исходной информацией для доказательства теоремы, оно окажется небольшим. Если же перейти к реальным прикладным задачам, размер дерева увеличится на несколько порядков.

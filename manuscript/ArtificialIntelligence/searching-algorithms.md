...
На первом шаге алгоритм A* выбирает между узлами B и C. Для узла B значение `f(n)` рассчитывается так:
```
f(n) = g(n) + h(n) = 20 + 35 = 55
```

Для узла C `f(n)` равно 45, согласно таблице 2-15. Поэтому алгоритм выберет узел с меньшим значением f(n), т.е. узел C.

Дальше алгоритм выбирает между узлами F и G. Вот расчёт функции `f(n)` для  узла F:
```
f(n) = g(n) + h(n) = 60 + 10 = 70
```

Это значение больше 45 для узла G. Поэтому алгоритм предпочтёт узел G.

Последний шаг очевиден — алгоритм предпочтёт переход в целевой узел H с `f(n) = 50`, чем в узел I с `f(n) = 60 + 40 = 100`.

...
Состязательный поиск работает не во всех проблемных средах. Чтобы он был эффективным, среда должна иметь следующие характеристики, согласно таблице 2-4:
...
Мы рассмотрим два алгоритма состязательного поиска. Их свойства указаны в таблице 2-17.

{caption: "Таблица 2-17. Алгоритмы состязательного поиска", width: "100%"}
...
Рассмотрим пример работы алгоритма минимакс. Иллюстрация 2-14 демонстрирует дерево игры и порядок обхода его узлов.

{caption: "Иллюстрация 2-14. Поиск по минимакс алгоритму"}
...
Таблица 2-18 демонстрирует шаги алгоритма минимакс для дерева игры на иллюстрации 2-14. При посещении каждого узла алгоритм вычисляет для него новое минимаксное значение. Оно так же приводится в таблице.

{caption: "Таблица 2-18. Шаги алгоритма минимакс", width: "70%"}
...
Согласно этому правилу, узлу D на иллюстрации 2-14 присваивается наибольшее минимаксное значение 6 дочернего узла I. Это происходит потому, что в узле D ходит игрок MAX. В узле B ходит игрок MIN, поэтому ему присваивается наименьшее минимаксное значение 6 дочернего узла D, а не значение 7 узла E. Аналогично алгоритм минимакс присваивает минимаксные значения всем узлам дерева.

После этого алгоритм составляет решение. Решение — это путь от корневого узла до терминального узла с наилучшим гарантированным выигрышем для игрока MAX. На иллюстрации 2-14 этот путь отмечен чёрной стрелкой:
...
[**Алгоритм альфа-бета отсечения**](https://ru.wikipedia.org/wiki/Альфа-бета-отсечение) — это модификация алгоритма минимакс, которая улучшает его временную сложность.

Алгоритм альфа-бета отсекает малоперспективные ветви. В лучшем случае это сокращает число проверяемых узлов наполовину. При этом результат поиска всегда такой же, как у минимакса.

Алгоритм альфа-бета отсечения получил своё названием из-за двух параметров, которые обновляются в процессе его работы:

* α (альфа) — наибольшее минимаксное значение узла, которое было найдено вдоль пути для игрока MAX. Это значение считается ограничением выигрыша MAX.

* β (бета) — наименьшее минимаксное значение узла, которое было найдено вдоль пути для игрока MIN. Оно будет ограничением выигрыша MIN.

Вот правила обновления значений α и β:

* Значение α обновляет только игрок MAX в узлах, в которых он делает ход. Новым значением α становится минимаксное значение проверяемого узла, только если оно больше текущего α. В противном случае α не меняется.

* Значение β обновляет только игрок MIN в узлах со своим ходом. Новым β становится минимаксное значение проверяемого узла, только если оно меньше текущего β. В противном случае β не меняется.

Отсечение происходит по следующему правилу:

* Алгоритм посещает очередной узел N. Если после обновления α или β выполняется неравенство α > β, все непроверенные дочерние узлы N отсекаются.

Ориентируясь на текущие значения α и β, алгоритм способен отсекать не только отдельные узлы, но и целые поддеревья.

Рассмотрим альфа-бета отсечение на примере игры с иллюстрации 2-14. Работу алгоритма демонстрирует иллюстрация 2-15.

{caption: "Иллюстрация 2-15. Поиск по алгоритму альфа-бета отсечения"}
![Алгоритм альфа-бета](images/ArtificialIntelligence/alpha-beta-pruning-tree.png)

Иллюстрация 2-15 показывает, что алгоритм отсекает узлы K, G, N, O. Они никак не влияют на конечное решение.

Таблица 2-19 демонстрирует шаги алгоритма для дерева игры на иллюстрации 2-15. Для каждого шага указаны: посещаемый узел, его новое минимаксное значение, текущие значения α и β.

{caption: "Таблица 2-19. Шаги алгоритма альфа-бета отсечения", width: "90%"}
...
Рассмотрим шаги алгоритма в таблице 2-19. Первые 11 шагов альфа-бета отсечения и минимакса полностью совпадают. Оба алгоритма присваивают одни и те же минимаксные значения узлам B, D, E, H, I, J.

Первое различие в таблицах 2-18 и 2-19 происходит на шаге 12. На этом шаге алгоритм минимакс посещает узел K. Альфа-бета отсечение пропускает узел K без проверки. Это происходит потому, что на 11 шаге в узле E выполнилось условие α > β (7 > 6). Из-за этого все непосещённые дочерние узлы E отсекаются.

Далее на 13 шаге алгоритм альфа-бета возвращается в корневой узел A. Он проходит по узлам C, F, L и M. В результате они получают те же минимаксные значения, которые им назначил бы минимаксный алгоритм.

На 20 шаге алгоритм альфа-бета поднимается из узла F в C. Алгоритм сравнивает текущие значения α и β. Для них выполняется условие α > β (6 > 5). Поэтому алгоритм отсекает все оставшиеся непосещённые дочерние узлы C: G, N и O.

В результате алгоритм альфа-бета находит такое же решение, как и алгоритм минимакс. Оно отмечено чёрной стрелкой на иллюстрации 2-15:

A -> B -> D -> I

Алгоритм альфа-бета отсечения решил задачу за 21 шаг. Минимакс сделал то же самое за 29 шагов. В этом и заключается преимущество альфа-бета отсечения.

Достоинства алгоритма альфа-бета отсечения:
...
Недостатки алгоритма альфа-бета отсечения:
...
* Повторяющиеся состояния в дереве игры (транспозиции) увеличивают временную сложность экспоненциально.

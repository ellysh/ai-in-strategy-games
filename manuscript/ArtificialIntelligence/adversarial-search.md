...
|  | | | | |
| Альфа-бета отсечение (alpha-beta pruning) | O(b<sup>m/2</sup>) | O(b*m) | Да | Да, если все оппоненты действуют оптимально |
...
Самый известный алгоритм состязательного поиска называется [**минимакс**](https://ru.wikipedia.org/wiki/Минимакс) (minimax). Он основан на [теореме о минимаксе](https://fr-m-wikipedia-org.translate.goog/wiki/Théorème_du_minimax_de_von_Neumann?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp). Джон фон Нейман доказал эту теорему в рамках теории игр в 1928 году.

>>>

Поиск минимакс проходит узлы дерева в том же порядке, что и поиск в глубину (DFS). Во время прохода алгоритм оценивает потенциальные выигрыши каждого игрока в посещённых узлах. Эти выигрыши называются **минимаксными значениями**.

...
Иллюстрация 2-4 демонстрирует ориентированный граф с двумя состояниями среды для программы очистки диска из раздела 2.2.4.

{caption: "Иллюстрация 2-4. Пространство состояний для программы очистки диска", height: "50%", width: "100%"}
...
В этом списке нет набора всех возможных состояний среды. Если этот набор известен, то пространство состояний представляют в виде **явного графа**. Граф такого типа помещается в память компьютера целиком. Он подходит для простых проблемных сред. Например, как на иллюстрации 2-4.
...
Успешная проверка означает, что агент нашёл искомое состояние среды. В этом случае путь от начального состояния к искомому уже находится в памяти компьютера и известен агенту.
...
Иллюстрация 2-5 демонстрирует графики самых распространённых функций оценки сложности алгоритма.

{caption: "Иллюстрация 2-5. Функции оценки сложности алгоритма", height: "30%"}
...
Пространственная сложность оценивается той же самой функцией O, что и временная. Только в случае пространственной сложности значение функции определяет объём используемой памяти. Другими словами, ось N на иллюстрации 2-5 покажет объём памяти, который использует алгоритм в процессе работы.
...
В общем случае алгоритмы неинформированного поиска работают для явных и неявных деревьев. Мы рассмотрим только случай явных деревьев. Так будет проще проиллюстрировать принцип работы алгоритмов.
...
Иллюстрация 2-6 демонстрирует порядок обхода узлов дерева.

{caption: "Иллюстрация 2-6. Порядок обхода узлов при поиске в ширину", height: "25%"}
...
* Требует много памяти (пространственная сложность). В худшем случае надо сохранить все посещённые узлы дерева.
...
Иллюстрация 2-7 демонстрирует порядок обхода узлов дерева.

{caption: "Иллюстрация 2-7. Порядок обхода узлов при поиске по критерию стоимости", height: "25%"}
...
{caption: "Иллюстрация 2-8. Порядок обхода узлов при поиске в глубину", height: "25%"}
...
{caption: "Иллюстрация 2-9. Порядок обхода узлов при поиске с ограничением глубины"}
...
{caption: "Иллюстрация 2-10. Порядок обхода узлов при поиске в глубину с итеративным углублением"}
![Поиск в глубину с итеративным углублением](images/ArtificialIntelligence/iddfs-tree.png)

Поиск выполняется как последовательность итераций. Таблица 2-12 демонстрирует итерации для дерева на иллюстрации 2-10.
...
{caption: "Иллюстрация 2-11. Порядок обхода узлов при двунаправленном поиске"}
...
Алгоритм останавливается на 6-й итерации. На этой итерации прямой и обратный поиск встречаются в узле E. Прямой поиск нашел такой путь от D до E: D -> B -> E. Обратный поиск нашёл такой путь от J до E: J -> F -> E. Комбинация этих двух путей будет конечным решением: D -> B -> E -> F -> J. Этот путь отмечен чёрной стрелкой на иллюстрации 2-11.
...
{caption: "Иллюстрация 2-12. Порядок обхода узлов при жадном поиске по первому наилучшему совпадению"}
...
Он отмечен чёрной стрелкой на иллюстрации 2-12.
...
{caption: "Иллюстрация 2-13. Порядок обхода узлов при поиске А\*"}
...
Этот путь отмечен чёрной стрелкой на иллюстрации 2-13.

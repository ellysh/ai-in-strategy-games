...
Мы познакомились с общими принципами формальной логики. Теперь перейдём к методам ИИ, которые используют алгоритмы поиска. Эти методы применяют интеллектуальные агенты двух типов:

* Агенты, основанные на цели.
* Агенты, основанные на производительности.

Сначала рассмотрим более простой классом алгоритмов, которые известны как **классический поиск**. Их применяют оба типа агентов, когда они работают в одноагентных средах.

### 2.4.1 Пространство состояний

В ИИ применяют алгоритмы [**поиска в пространстве состояний**](https://ru.wikipedia.org/wiki/Поиск_в_пространстве_состояний). Они отличаются от [традиционных алгоритмов поиска](https://en.wikipedia.org/wiki/Search_algorithm), которые изучает информатика.
...
Иллюстрация 2-6 демонстрирует ориентированный граф с двумя состояниями среды для программы очистки диска из прошлого раздела.
...
Следующие компоненты определяют пространство состояний:
...
В этом списке нет набора всех возможных состояний среды. Если этот набор известен, то форма представления пространства состояний называется **явным графом**. Граф такого типа помещается в память компьютера целиком. Он подходит для простых проблемных сред, как на иллюстрации 2-6.
...
Иллюстрация 2-7 демонстрирует графики часто встречающихся функций оценки сложности алгоритма. Графики отражают зависимость числа шагов алгоритма N от размера входных данных n
...
Пространственная сложность оценивается теми же самыми функциями, что и временная. В случае пространственной сложности ось N на иллюстрации 2-7 показывает объём памяти (например, в мегабайтах), который использует алгоритм в процессе своей работы.
...
3. Поиск в условиях противодействия или состязательный поиск (adversarial search)

Рассмотрим алгоритмы каждого класса и сравним их эффективность.

### 2.4.4 Неинформированный поиск

К **неинформированному поиску** относятся алгоритмы, которые не используют дополнительную информацию о пространстве поиска. В этих алгоритмах выполняются только два типа действий:
...
Алгоритмы неинформированного различаются между собой только порядком обхода вершин графа пространства состояний. Эти алгоритмы применяют основанные на цели агенты.

Таблица 2-9 демонстрирует самые распространённые алгоритмы неинформированного поиска.

{caption: "Таблица 2-9. Алгоритмы неинформированного поиска", width: "100%"}
...
Иллюстрация 2-11 демонстрирует порядок обхода узлов дерева поиска.
...
Иллюстрация 2-12 демонстрирует порядок обхода узлов дерева поиска.
...
Поиск выполняется как последовательность итераций. Таблица 2-10 демонстрирует итерации для дерева поиска на иллюстрации 2-12.

{caption: "Таблица 2-10. Итерации поиска", width: "80%"}
...
**Двунаправленный поиск** (bidirectional search) заключается в одновременном поиске из начального и целевого узлов. Поиск из начального узла выполняется в прямом направлении, а из целевого узла — в обратном. Алгоритм прекращается, когда два процесса поиска встречаются. Таким образом решение составляется из двух частей. Это части пути, найденные поиском в прямом и в обратном направлениях.
...
Иллюстрация 2-13 демонстрирует порядок обхода узлов дерева поиска.
...
Во время двунаправленного поиска чередуются итерации прямого и обратного поиска. Таблица 2-11 демонстрирует эти итерации.

{caption: "Таблица 2-11. Итерации двунаправленного поиска", width: "80%"}
...
Алгоритм останавливается на 6-ой итерации, когда прямой и обратный поиск встречаются в узле E. Далее алгоритм объединяет оба результата в один: D -> B -> E -> F -> J. Этот путь отмечен на иллюстрации 2-13 чёрной стрелкой.
...
Мы рассмотрели алгоритмы неинформированного поиска. Их общая проблема в неэффективности для большинства реальных задач. Агент под управлением такого алгоритма не способен работать в сложной среде. В такой среде пространство поиска становится большим. Для его обработки неинформированный поиск требует слишком много времени и компьютерной памяти.

Алгоритмы **информированного поиска** решают проблему обработки большого пространства поиска. Их применяют агенты, основанные на полезности.

При информированном поиске агент использует дополнительную информацию о поставленной задаче. Вот несколько примеров такой информации:
...
Таблица 2-12 демонстрирует алгоритмы информированного поиска, которые применяются на практике чаще всего.

{caption: "Таблица 2-12. Алгоритмы информированного поиска", width: "100%"}
| Алгоритм | Время | Память | Полнота | Оптимальность |
| --- | --- | --- | --- | --- |
...
Иллюстрация 2-14 демонстрирует порядок обхода узлов дерева поиска.
...
Таблица 2-13 демонстрирует все значения эвристической функции.

{caption: "Таблица 2-13. Значения эвристической функции", width: "50%"}
...
Жадный поиск по первому наилучшему совпадению ориентируется только на значения эвристической функции из таблицы 2-13. Поэтому алгоритм найдёт следующий путь:

A -> C -> F -> H

Он отмечен чёрной стрелкой на иллюстрации 2-14.

На первом шаге алгоритм выбирает между узлами B и C. Согласно эвристической функции из таблицы 2-13, узел C находится ближе к целевому узлу, чем B. Расстояние C-H равно 25, а B-H 35.

Далее алгоритм выбирает между узлами F и G. Расстояние F-H равно 10, согласно таблице 2-13. Расстояние G-H равно 15. Это значит, что узел F ближе к узлу H. Поэтому алгоритм выберет путь через узел F.
...
Иллюстрация 2-15 демонстрирует порядок обхода узлов дерева поиска.

{caption: "Иллюстрация 2-15. Порядок обхода узлов при поиске А\*"}
![Поиск А\*](images/ArtificialIntelligence/a-star-tree.png)

Пусть значения эвристической функции будут такими же, как в таблице 2-13. В этом случае алгоритм поиска выполнит шаги из таблицы 2-14.

{caption: "Таблица 2-14. Шаги поиска A\*", width: "80%"}
...
Таким образом поиск А* найдёт следующее решение:

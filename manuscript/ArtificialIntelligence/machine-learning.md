...
3. **Подход, основанный на модели** (model-based)  создаёт виртуальной модели среды. Агент исследует эту модель и таким образом изучает среду. После обучения агент использует полученную на модели стратегию для выбора действий в реальной среде. Этот подход обычно применяется для агентов, работающих в физической среде. Физические процессы, например перемещение робота-пылесоса по комнате, протекают медленно. Модель среды позволяет многократно повторять их в ускоренном режиме. Поэтому алгоритм обучения выполняется в разы быстрее, чем это происходило бы в физической среде.
...
Рассмотренный нами метод называется [обучением с временной разницей](http://www.scholarpedia.org/article/Temporal_difference_learning) (temporal difference learning). Семейство подобных методов называется [Q-learning](https://en.wikipedia.org/wiki/Q-learning).

Кроме Q-learning сегодня популярны следующие методы:
...
В этой [статье](https://habr.com/ru/post/561746/) приводится их краткое описание.
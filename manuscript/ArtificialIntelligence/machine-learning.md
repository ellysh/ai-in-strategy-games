...
Алгоритмы обучения с учителем и без учителя создают модели, которые работают в эпизодических средах. Обучение происходит на несвязанных между собой примерах входных данных. Готовые модели обрабатывают такие же несвязанные новые данные. В этом случае прошлые решения агента никак не влияют на его последующие решения. Поэтому агент не отслеживает долгосрочный результат своих действий. В результате он не способен действовать в последовательной среде. Для этого нужна другая парадигма, а именно обучение с подкреплением.

Главные компоненты обучения с подкреплением — это агент и среда. Агент последовательно выполняет действия, которые меняют состояние среды. Он может оценить каждое выполненное им действие. Таким образом, агент отличает успешные и неуспешные действия. Не имея первоначальной информации о способе решить задачу, агент вынужден искать решение [методом проб и ошибок](https://en.wikipedia.org/wiki/Trial_and_error).
...
* **Сигнал вознаграждения** (reward signal) определяет цель обучения с подкреплением. Среда посылает агенту этот сигнал в ответ на каждое выполненное им действие. Вознаграждение представляет собой число. За успешные действия агент получает большее вознаграждение, чем за менее успешные. Главная цель агента — максимизировать своё суммарное вознаграждение в долгосрочной перспективе. Ориентируясь на вознаграждение, агент меняет свою стратегию. Таким образом агент избегает действий с низким вознаграждением.
...
I> Обратите внимание на различие между терминами "модель окружения" и "обучаемая модель". Вместо обучаемой модели в терминологии обучения с подкреплением применяется понятие "агент".

Алгоритм обучения с подкреплением получает на вход следующие данные:
...
2. **Подход, основанный на стратегии** (policy based) ищет оптимальную стратегию без использования функции полезности. Следуя такой стратегии, агент получает максимально возможное итоговое вознаграждение за свои действия. Сами стратегии бывают двух типов:
...
// TODO: Классический пример - решение лабиринта, который не знаком агенту.
...
Чтобы воспользоваться механизмом обучения, нужен **обучающий набор данных**. Этот набор состоит из обучающих примеров.
...
Алгоритмы обучения с учителем принимают на вход **размеченные данные**. Каждый обучающий пример в таких данных представляет собой акт восприятия агента и ожидаемое действие на него. Эти ожидаемые действия агента называются **метками**.

Пример размеченных данных — набор картинок с животными. У каждой картинки есть метка. Она называет изображённое животное: кот или собака. После обучения на таком наборе данных мы ожидаем, что агент научится отличать собак от кошек на картинках самостоятельно.

Принимая на вход размеченные данные, алгоритм обучения наблюдает за примерами работы желаемой функции агента. В результате обучения алгоритм строит модель. Она реализует функцию агента, поведение которой близко к желаемой.

Во время обучения модель пытается самостоятельно определить результат для каждого обучающего примера. Если результат не совпал с меткой, это называется [**ошибкой обучения**](https://wiki.loginom.ru/articles/training-error.html) (training error). При возникновении такой ошибки алгоритм обучения автоматически корректирует модель и проверяет её снова.

Чтобы оценить уже готовую модель, применяется размеченный **тестовый набор данных**. Эти данные не входят в обучающий набор, поэтому модель никогда с ними не встречалась. Если результаты модели не совпадают с метками, это называют [**ошибками обобщения**](https://wiki.loginom.ru/articles/generalization-error.html) (generalization error). При высокой частоте таких ошибок, говорят что **производительность** модели низка.

Чтобы улучшить производительность модели, есть целый [ряд методов](https://machinelearningmastery.com/improve-deep-learning-performance/). Самый простой из них: поменять параметры модели и повторить процесс обучения.

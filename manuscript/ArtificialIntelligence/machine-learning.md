...
Задача кластеризации возникает, когда нужно сгруппировать примеры входных данных. Эти входные данные не размечены. Поэтому алгоритм обучения не знает, на к каким именно группам следует относить примеры. Алгоритм должен сам выбрать подходящие группы на основе различаемых признаков объектов. Важное условие кластеризации: полученные группы объектов не должны пересекаться. Другими словами, каждый объект должен относится только к одной группе.
...
Рассмотрим задачу регрессии на примере. Предположим, что онлайн магазину нужно сгруппировать пользователей по интересом. Это позволит показывать каждому пользователю только интересную ему рекламу о новых товарах.

У магазина есть статистика по просмотрам пользователями разных категорий товаров. Для простоты рассмотрим только две категории товаров: велозапчасти и автозапчасти.
...
Применим один из популярных алгоритмов обучения для задачи кластеризации под названием [**метод k-средних**](https://en.wikipedia.org/wiki/K-means_clustering) (k-means). Этому алгоритму нужно указать число ожидаемых кластеров. В нашем примере это число равно четырём. Именно столько вариантов показать рекламу есть у онлайн магазина.

Метод k-средних в ходе обучения находит [**геометрический центр**](https://ru.wikipedia.org/wiki/Барицентр) (centroid) каждого кластера на пространстве признаков. Геометрический центр множества точек представляет собой среднее арифметическое координат этих точек.

Найденные центры кластеров являются обучаемой моделью. Предположим, что обученная модель получает на вход набор признаков незнакомого объекта. Она вычисляет расстояние от объекта до центра каждого кластера на пространстве признаков. Ближайший к объекту центр определит, к какому кластеру его отнести.

Для наглядности представим обученную модель для нашего примера на графике. Поскольку мы рассматриваем только два признака пользователей, пространство признаков будет двумерным. Его демонстрирует иллюстрация 2-16.

...
**Вес признаков** (feature weight) w~i~ — это коэффициенты, которые подбирает алгоритм обучения. Если правильно подобрать веса, пороговая функция сможет правильно определять класс объекта по заданным признакам.
...
Возникает вопрос: какая именно пороговая функция f используется в формуле? Эта функция зависит от конкретной обучаемой модели, которую мы решим использовать. 
...
При x = 0 график проходит через значение 0.5 на оси y. Чем больше входное значение x, тем ближе результат функции y к значению 1. Чем меньше значение x, тем ближе y к 0.
...
В нашем случае признаков два, поэтому график строится в двумерном пространстве. Его демонстрирует иллюстрация 2-19.

{caption: "Иллюстрация 2-19. Модель для классификации ирисов", height: "50%", width: "100%"}
...
Иллюстрация 2-20 демонстрирует модель линейной регрессии для определения цены Volkswagen Golf.

{caption: "Иллюстрация 2-20. Модель для вычисления стоимости автомобиля", height: "50%", width: "100%"}
...
Вес w~1~ определяет наклон линии на иллюстрации 2-20.
...
Для наглядности представим обученную модель для нашего примера с онлайн магазином на графике. У каждого пользователя мы учитываем только два признака: посещения двух категорий товаров. Поэтому пространство признаков на графике будет двумерным. Его демонстрирует иллюстрация 2-21.

{caption: "Иллюстрация 2-21. Модель для кластеризации пользователей онлайн магазина", height: "50%", width: "100%"}

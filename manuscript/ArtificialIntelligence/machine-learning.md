...
Чтобы продемонстрировать работу модели, обратимся к графику на иллюстрации 2-14. Предположим, что модель получила входные данные: ширина и длина чашелистника неизвестного ириса. Отложим на горизонтальной и вертикальной оси значения этих признаков. Если точка оказалась выше границы классов, проверяемый ирис относится к виду Iris setosa. В противном случае, этот ирис относится к другому виду.
...
Рассмотрим, как работает наша модель регрессора. На вход она получает вектор независимых переменных X. Он состоит из единственного значения x~1~ — пробег автомобиля, цену которого надо определить. Модель подставляет это значение x~1~ в формулу и вычисляет y. Значение y — это искомая цена автомобиля.

Обратимся к графику на иллюстрации 2-15, чтобы представить работу модели наглядно. Найдём цену автомобиля, исходя из его пробега. Для этого отложим величину пробега на горизонтальной оси. Например, это 2000 км. Такому пробегу соответствует одна точка на синей линии. По вертикальной оси этой точке соответствует цена 20000$. Таким образом мы нашли ожидаемую цену автомобиля.
...
Может возникнуть вопрос: если данные не размечены, как алгоритм обучения поймёт что с ними делать? Обучение без учителя решает совершенно иные задачи, чем обучение с учителем. Его цели можно сформулировать так:
...
Эволюционные алгоритмы плохо справляются со сложными задачами. Кроме того они требуют значительных вычислительных ресурсов и времени. Сегодня более популярны следующие методы, основанные на стратегии (policy-based):

* [Policy gradient](https://jonathan-hui.medium.com/rl-policy-gradients-explained-9b13b688b146) — алгоритм обучения увеличивает вероятность выбора стратегий дающих высокое итоговое вознаграждение. Одновременно с этим вероятность выбора стратегий с низким вознаграждением понижается. Метод policy gradient комбинируется с другими алгоритмами конструирования стратегий.

* [Advantage actor-critic](https://julien-vitay.net/deeprl/ActorCritic.html) (A2C) — алгоритм обучения состоит из двух компонентов: actor (действующий объект) и critic (critic). Actor выполняет несколько действий, согласно выбранной стратегии. Critic вычисляет полезность состояний среды, которые стали доступны после действий actor. В зависимости от результатов critic, actor меняет улучшает свою стратегию. После выполнения действий actor и расчёт фактических вознаграждений в новых состояниях среды, critic улучшает свою оценку полезности.

* [Asynchronous advantage actor-critic](https://julien-vitay.net/deeprl/ActorCritic.html) (A3C) — асинхронный вариант алгоритма A2C. В нём несколько пар actor-critic работают параллельно. Они обрабатывают разные стратегии и доступные в них состояния среды. Результаты их работы накапливаются в виде финальной стратегии.

* Proximal Policy Optimization (PPO)

* Trust Region Policy Optimization (TRPO)

<TODO: Дать краткое описание этих методов и ссылки на источники>

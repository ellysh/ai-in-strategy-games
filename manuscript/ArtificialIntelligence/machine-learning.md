...
Дилемму исследования и эксплуатации хорошо иллюстрирует проблема многорукого бандита. [**Проблема многорукого бандита**](https://en.wikipedia.org/wiki/Multi-armed_bandit) — это классическая задача обучения с подкреплением. Проблема получила своё название из аналогии с игроком, который стоит перед рядом игровых автоматов типа однорукий бандит. Одноруким бандитом называется [слот-машина](https://ru.wikipedia.org/wiki/Слот-машина) с рычагом и барабаном. Пользователь должен бросить монету и дёрнуть за рычаг. Тогда запускается барабан, который при остановке показывает комбинацию символов. Некоторые комбинации являются выигрышными. При их выпадении пользователь получает сумму, превышающую размер ставки.
...
Задача агента — последовательно сыграть на автоматах на все имеющиеся у него монеты и получить максимально возможный выигрыш. Другими словами, он должен максимизировать своё совокупное вознаграждение.
...
Общего решения для всех задач этого класса найти ещё не удалось. Но учёные уже разработали оптимальные стратегии для частных случаев. Кроме них есть ряд универсальных приблизительных стратегий. Они дают не наилучший, но приемлемый результат.

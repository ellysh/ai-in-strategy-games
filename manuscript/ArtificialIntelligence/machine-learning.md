...
1. Вектор входных значений X содержит все проверяемые признаки конкретного объекта.
...
* 0 — виду Iris setosa.
* 1 — другим видам ирисов.

**Вес признаков** (feature weight) w~i~ — это коэффициенты, которые подбирает алгоритм обучения. Если правильно подобрать веса, пороговая функция сможет определять класс объекта по заданным признакам.

В примере с ирисами мы учитываем только два признака: ширина и длина чашелистика цветка. Поэтому формулу линейного классификатора можно упростить:
{width: "35%"}
...
Возникает вопрос: какую именно пороговую функцию f использует линейный классификатор? Эта функция зависит от выбранной обучаемой модели.

На практике для линейной классификации применяют следующие модели:
...
Это список самых популярных моделей. Кроме них есть и другие.
...
В нашем примере только два признака. Поэтому граница принятия решений строится в двумерном пространстве. Её демонстрирует иллюстрация 2-20.
...
На горизонтальной оси откладывается длина чашелистик в сантиметрах для каждого примера из обучающего набора данных. На вертикальной оси — ширина чашелистик. Фиолетовые точки в левой верхней части графика обозначают ирисы вида Iris setosa. Зелёные и жёлтые точки справ внизу — это ирисы видов Iris virginica и Iris versicolor. Синяя пунктирная линия соответствует границе между классами. Это и есть граница принятия решений. Её наклон определяют веса w~1~ и w~2~, которые подобрал алгоритм обучения.
...
График на иллюстрации 2-20 наглядно демонстрирует работу модели. Допустим, модель получила входной вектор X. Отложим на горизонтальной и вертикальной оси значения признаков x~2~ и x~1~. Получим точку в пространстве. Она соответствует цветку некоторого ириса, который надо классифицировать. Если точка оказалась выше границы принятия решений, растение относится к виду Iris setosa. В противном случае, это другой вид ириса.

Остаётся открытым ещё один вопрос. Как именно алгоритм обучения подбирает веса w~1~ и w~2~? Для моделей линейных классификаторов часто применятся [**стохастический градиентный спуск**](https://ru.wikipedia.org/wiki/Стохастический_градиентный_спуск) (stochastic gradient descent или SGD). Этот алгоритм реализован в нескольких библиотеках с открытым исходным кодом. Например, [scikit-learn](https://scikit-learn.org/stable/modules/sgd.html) и [TensorFlow](https://www.tensorflow.org/api_docs/python/tf/keras/optimizers/experimental/SGD). На практике применяют одну из этих готовых реализаций.

Приложение 5.1.1 подробно описывает Python скрипт для классификации ирисов Фишера. Этот скрипт также доступен на [Github](https://github.com/ellysh/ai-in-strategy-games/blob/master/manuscript/resources/code/ArtificialIntelligence/linear-classifier.py).
...
Иллюстрация 2-21 демонстрирует модель линейной регрессии для определения цены Volkswagen Golf.

{caption: "Иллюстрация 2-21. Модель для вычисления стоимости автомобиля", height: "50%", width: "100%"}
...
Вес w~1~ определяет наклон линии на иллюстрации 2-21.
...
Для наглядности представим обученную модель для нашего примера с онлайн магазином на графике. У каждого пользователя мы учитываем только два признака: посещения двух категорий товаров. Поэтому пространство признаков на графике будет двумерным. Его демонстрирует иллюстрация 2-22.

{caption: "Иллюстрация 2-22. Модель для кластеризации пользователей онлайн магазина", height: "50%", width: "100%"}

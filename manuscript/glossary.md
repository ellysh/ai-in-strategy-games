...
**Дерево игры** отображает все возможные действия игроков и все исходы игры.
...
**Фактор ветвления** — это среднее количество вариантов действий у игроков на каждом     ходе.
...
**Аппроксимация** или приближение — это замена сложных объектов более простыми. При этом важные аспекты заменяющих и исходных объектов похожи. В контексте ИИ аппроксимация часто выполняется для функций и моделей.
...
**Градиент** (gradient) — в машинном обучении это скорость изменения функции по отношению к её входным переменным. Градиент даёт информацию о том, как небольшие изменения входных переменных влияют на результат функции.
...
**Метод Монте-Карло** —  широкий класс вычислительных алгоритмов, которые полагаются на повторную случайную выборку и статистический анализ для получения численных результатов.
...
[**Рекуррентная нейронная сеть**](https://ru.wikipedia.org/wiki/Рекуррентная_нейронная_сеть) (recurrent neural network или RNN) — сеть с замыкающимися связями. Эти связи обеспечивают двунаправленный поток информации — вперёд и назад: от входных узлов через скрытые узлы к выходным узлам и обратно.

### С

[**Свёрточная нейронная сеть**](https://ru.wikipedia.org/wiki/Свёрточная_нейронная_сеть) (convolutional neural network или CNN) — это подвид многослойной нейронной сети с прямой связью (FNN). Её архитектура использует особенность зрительной коры человека: чередование двух типов слоёв. Эта сеть оптимизирована для обработки изображений.
...
**Классический контроль времени** — на первые 40 ходов каждому игроку отводится 90 минут. После 40-ого хода каждому игроку добавляется 30 минут. К этим 30 минутам добавляется по 30 секунд за каждый ход, сделанный с начала партии.

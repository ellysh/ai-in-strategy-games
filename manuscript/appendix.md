...
* `SGDClassifier` из библиотеки `sklearn` — содержит набор моделей для линейной классификации. Кроме того в этом модуле реализован алгоритм обучения этих моделей методом [стохастического градиентного спуска](https://ru.wikipedia.org/wiki/Стохастический_градиентный_спуск).
...
Далее скрипт создаёт объект `sgdc` класса `SGDClassifier`. Конструктор класса получается два параметра:
...
6. `show` — вывести на экран всё содержимое всех объектов класса `Figure`.
...
![Формула границы разделения классов](images/Appendix/logreg-classes-border.png)
...
![Вычисление x2 по x1](images/Appendix/logreg-x2-calculation.png)

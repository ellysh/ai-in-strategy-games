...
В начале скрипта объявляется двумерный массив размера 3x3 с вертикальным фильтром Собеля. Этот массив сохраняется в объект `array` из библиотеки `numpy`. Объект предоставляет удобный интерфейс для работы с массивами.

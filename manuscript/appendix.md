...
Скрипт для обучения агента игре в крестики-нолики большой. Поэтому сначала познакомимся с общим принципом его работы. Затем рассмотрим каждую часть по отдельности.

В скрипте есть два класса:
...
Кроме этих классов есть две функции:

* `train_q_learning_agent` — запускает алгоритм обучения двух агентов. Эти агенты играют против друг друга. По результатам каждой игры они обновляют свои таблицы Q-значений.
...
4. Провести серию игр между обученными агентами. Для каждой игры вывести на экран ходы агентов и результат. Для этого вызвать функцию `test_q_learning_agent`.

Листинг демонстрирует реализацию класса среды `TicTacToe`.

{caption: "Листинг 5-4. Реализация класса TicTacToe", line-numbers: true, format: Python}
```
...
Класс `TicTacToe` имеет следующие поля:

* `board` — двумерный массив размером 3x3. Он содержит текущее состояние игрового поле. Изначально массив занулён. Ход игрока X записывается как 1 в соответствующее поле. Ход игрока O записывается как -1.

* `current_player` — число, которое определяет какой игрок сейчас делает ход. Оно равно 1 для игрока X и -1 для игрока O.
...
* `is_valid_move` — проверяет что указанный ход в указанное игровое поле допустим.

* `make_move` — выполняет ход текущего игрока в указанное игровое поле.
...
Класс среды QLearningAgent имеет следующие поля:

...
На втором шаге скрипт выполняет произведение Адамара. Для этого он объявляет следующие три массива:
...
После подготовки временных массивов скрипт выполняет [**вложенный цикл**](https://education.yandex.ru/handbook/python/article/vlozhennye-cikly). Внешний цикл выполняет следующий оператор `for`:
{line-numbers: false, format: Python}
```
for i in range(1, 4):
```
...
2. Выполнить произведение Адамара. Его результат представляет собой одно число. Оно записывается в конец массива `row`. Произведение Адамара выполняется за два шага:
   ...
   * Вызвать функцию `multiply` из библиотеки `numpy`. Она получает на вход два массива и перемножает их элементы с одинаковыми индексами. Это означает, что первый элемент умножается на первый, второй на второй и т.д. Результат функции возвращается в виде массива в `ndarray` объекте.

   * Вызвать функцию `sum` из библиотеки `numpy`. Она получает на вход один объект `ndarray` с массивом и скалывает все его элементы между собой.
...
После завершения внешнего цикла `for` результат произведения Адамара хранится в массиве `result`. Скрипт выводит его на консоль с помощью функции `print`. Этот вывод выглядит следующим образом:
{line-numbers: false, format: Python}
```

...
После подготовки обучающего и тестового набора данных, скрипт создаёт модель линейной регрессии. Она реализована в классе `LinearRegression` из модуля `linear_model`. Скрипт создаёт объект этого класса и сохраняет его в переменной с именем `regr`.
...
Функция возвращает карту признаков с результатом операции свёртки. Она представляет собой объект класса `ndarray`, который сохраняется во временную переменную с именем `result`. Затем переменная добавляется в список с именем `channels`.
...
В начале скрипта объявляется двумерный массив `sobel_vertical` размера 3x3 с вертикальным фильтром Собеля. Он помещается в объект `ndarray` из библиотеки `numpy`.

Далее объявляется массив `image` размера 5x5 c пикселями исходного чёрно-белого изображения. Он также помещается в объект `ndarray`.
...
* Вызов функции `multiply` из библиотеки `numpy`. Она получает на вход два массива и перемножает их элементы с одинаковыми индексами. Это означает, что первый элемент умножается на первый, второй на второй и т.д. Результат функции возвращается в виде массива в `ndarray` объекте.
